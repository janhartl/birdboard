...
fn build_independent_equilibrium_market() -> Result<()> {
    ...
    let requested_profiles = std::env::var("BIRDBOARD_MARKET_PROFILES")
        ...
        .unwrap_or(16)
        .max(TEAM_COUNT);
    // v74 treats this as a hard cap, not a requested fixed epoch count.  The
    // solver exits early once the economically important part of the curve is
    // stable.  Ten damped epochs are still only ~1-2 minutes on the current
    // dataset, while being much safer than stopping after three oscillatory
    // Picard iterations.
    let max_epochs = std::env::var("BIRDBOARD_MARKET_EPOCHS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(10)
        .clamp(2, 20);
    let damping = std::env::var("BIRDBOARD_MARKET_DAMPING")
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .unwrap_or(0.50)
        .clamp(0.10, 1.00);
    let top100_target = std::env::var("BIRDBOARD_MARKET_TOP100_MAD")
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .unwrap_or(0.50)
        .max(0.05);
    let top30_target = std::env::var("BIRDBOARD_MARKET_TOP30_MAX")
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .unwrap_or(1.50)
        .max(0.10);
    ...
    println!("\nBirdBoard independent rational-bid equilibrium market v74");
    println!(
        "{} players | {} representative manager profiles | {} synthetic 13-manager rooms | max {} damped fixed-point epochs",
        candidate_pool.len(),
        profiles.len(),
        rooms,
        max_epochs
    ...
    println!(
        "damping alpha {:.2}; convergence: top-100 MAD < ${:.2}, top-30 max move < ${:.2}, top-30 membership >= 28/30",
        damping, top100_target, top30_target
    );
    println!("ESPN inputs: NONE\n");

    // Start from an explicit legacy statistical price vector so epoch 1 can be
    // damped too.  Calling market_board() is deliberate: that function is the
    // legacy G/$ market and ignores any previously saved equilibrium file.
    let empty_opponent_rosters = vec![Vec::<PlayerId>::new(); TEAM_COUNT.saturating_sub(1)];
    let full_opponent_budgets = vec![config.starting_budget; TEAM_COUNT.saturating_sub(1)];
    let legacy_board = app.durant.market_board(
        &[],
        config.starting_budget,
        &empty_opponent_rosters,
        &full_opponent_budgets,
        &candidate_pool,
        config,
    );
    let mut prior_prices = legacy_board
        .values
        .iter()
        .map(|value| {
            (
                value.player_id.clone(),
                value.market_price_exact.max(MIN_BID as f64),
            )
        })
        .collect::<HashMap<_, _>>();
    if prior_prices.len() != candidate_pool.len() {
        bail!(
            "legacy initialization priced {} of {} candidate players",
            prior_prices.len(),
            candidate_pool.len()
        );
    }
    ...
    for epoch in 0..max_epochs {
        ...
        // Convergence is measured against the current fixed-point state, not
        // averaged over 600 mostly-cheap players.  That prevents the tail from
        // hiding $5-$15 oscillations among the stars that actually determine
        // auction strategy.
        let top100_count = rows.len().min(100);
        let top30_count = rows.len().min(30);
        let top100_mad = if top100_count == 0 {
            0.0
        } else {
            rows.iter()
                .take(top100_count)
                .map(|row| {
                    let id = PlayerId(row.player_id.clone());
                    let old = prior_prices
                        .get(&id)
                        .copied()
                        .unwrap_or(row.equilibrium_price);
                    (row.equilibrium_price - old).abs()
                })
                .sum::<f64>()
                / top100_count as f64
        };
        let top30_max_change = rows
            .iter()
            .take(top30_count)
            .map(|row| {
                let id = PlayerId(row.player_id.clone());
        ...
        let mut prior_order = candidate_pool.clone();
        prior_order.sort_by(|left, right| {
            prior_prices
                .get(right)
                .copied()
                .unwrap_or(MIN_BID as f64)
                .total_cmp(&prior_prices.get(left).copied().unwrap_or(MIN_BID as f64))
                .then_with(|| left.0.cmp(&right.0))
        });
        let prior_top30 = prior_order
            .iter()
            .take(top30_count)
            .enumerate()
            .map(|(rank, player_id)| (player_id.clone(), rank))
            .collect::<HashMap<_, _>>();
        let mut overlap = 0usize;
        let mut rank_shift_sum = 0usize;
        for (rank, row) in rows.iter().take(top30_count).enumerate() {
            let id = PlayerId(row.player_id.clone());
            if let Some(old_rank) = prior_top30.get(&id) {
                overlap += 1;
                rank_shift_sum += rank.abs_diff(*old_rank);
            }
        }
        let mean_rank_shift = if overlap == 0 {
            top30_count as f64
        } else {
            rank_shift_sum as f64 / overlap as f64
        };

        println!(
            "epoch {} finished in {:.1}s | top-100 MAD ${:.2} | top-30 max ${:.2} | top-30 overlap {}/{} | mean rank shift {:.2}",
            epoch + 1,
        ...
        let price_stable = top100_mad < top100_target && top30_max_change < top30_target;
        let order_stable = top30_count < 30 || (overlap >= 28 && mean_rank_shift <= 2.0);
        if price_stable && order_stable {
            println!("  CONVERGED: economically important price curve is stable.\n");
            final_rows = rows;
            converged = true;
            break;
        }

        // Damped Picard update: P_(n+1) = (1-alpha) P_n + alpha F(P_n).
        // This fixes the high-low-high oscillation observed in v73 without
        // changing any basketball valuation or fitting to ESPN.
        let mut damped_prior = HashMap::<PlayerId, f64>::with_capacity(raw_prior.len());
        for player_id in &candidate_pool {
            let old = prior_prices
                .get(player_id)
                .copied()
                .unwrap_or(MIN_BID as f64);
            let raw = raw_prior.get(player_id).copied().unwrap_or(old);
            damped_prior.insert(
                player_id.clone(),
                ((1.0 - damping) * old + damping * raw).max(MIN_BID as f64),
            );
        }
        prior_prices = damped_prior;
        final_rows = rows;
        println!("  damped prior updated with alpha {:.2}\n", damping);
    }

    if !converged {
        bail!(
            "equilibrium market did not converge within {} epochs; refusing to overwrite the saved market. Increase BIRDBOARD_MARKET_EPOCHS or lower BIRDBOARD_MARKET_DAMPING (current {:.2}).",
            max_epochs,
            damping
        );
    }
    ...
    println!("Saved converged independent market to {}", path.display());
    ...
    println!(
        "\nThese are converged demand weights from rational reservation bids, not ESPN-fitted prices. Normal BirdBoard rescales them to the real room's remaining discretionary dollars."
    );
    Ok(())
}
