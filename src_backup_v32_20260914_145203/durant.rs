...
impl DurantModel {
    ...
    pub(crate) fn roster_plan_validation_common_market_pair(
        ...
        if own_roster.len() > self.team_size || bank_strategy_weights.is_empty() {
            return None;
        }
        ...
        // Pricing is intentionally fixed to the runtime bank for BOTH search
        // spaces.  We pay the expensive all-player rational-market cost once
        // because this is offline validation, not a live-draft code path.
        let pricing_strategies = bank_strategy_weights
            .iter()
            .copied()
            .map(|weights| RolloutStrategy { weights })
            .collect::<Vec<_>>();
        ...
        // Restricted/runtime-bank search under the common market.
