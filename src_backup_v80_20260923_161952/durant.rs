...
pub(crate) const H_CALIBRATION_FILENAME: &str = "h_calibration_v36.json";

/// Empirical calibration applied to the raw independent-category H score.
///
/// The runtime correction is intentionally one-dimensional and symmetric:
///     H_cal = logistic(slope * logit(H_raw))
/// A positive slope preserves every ordering and BUY/PASS indifference point;
/// slope < 1 simply compresses overconfident probabilities toward 50%.
#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub(crate) struct HCalibration {
    pub schema_version: u32,
    pub slope: f64,
}

impl Default for HCalibration {
    fn default() -> Self {
        Self {
            schema_version: 1,
            slope: 1.0,
...
impl HCalibration {
    pub(crate) fn from_slope(slope: f64) -> Self {
        Self {
            schema_version: 1,
            slope: if slope.is_finite() && slope > 0.0 {
                slope
            } else {
                1.0
            },
    ...
    pub(crate) fn apply(self, probability: f64) -> f64 {
        let p = probability.clamp(1.0e-12, 1.0 - 1.0e-12);
        if (self.slope - 1.0).abs() <= 1.0e-12 {
            return probability.clamp(0.0, 1.0);
        }
        let logit = (p / (1.0 - p)).ln();
        let z = self.slope * logit;
        if z >= 0.0 {
            1.0 / (1.0 + (-z).exp())
        } else {
            let ez = z.exp();
...
#[derive(Debug, Clone)]
pub(crate) struct StrategyFunnelPoint {
    pub k: usize,
    pub coarse_count: usize,
    pub focus_player_count: usize,
    pub projected_h: f64,
    pub seconds: f64,
}

#[derive(Debug, Clone)]
pub(crate) struct StrategyFunnelSweep {
    pub reference_h: f64,
    pub reference_seconds: f64,
    pub coarse_seconds: f64,
    pub points: Vec<StrategyFunnelPoint>,
}
...
pub struct AuctionDurantScore {
    ...
    /// Preparation / static market price shown on the live board.
    /// This is supplied by App for the single-candidate nomination audit.
    #[serde(default)]
    pub prep_market_price: u16,
    /// Price actually evaluated for the prep-market BUY diagnostic after
    /// clamping to our current legal maximum bid.
    #[serde(default)]
    pub evaluated_prep_market_price: u16,

    /// Opponent-aware competition / clearing price for this candidate.
    /// Historically this field was named market_price, so keep the serialized
    /// field for cache compatibility while the UI labels it explicitly.
    pub market_price_exact: f64,
    pub market_price: u16,
    /// Price actually used for the competition-price BUY diagnostics. This only
    ...
    /// Best completed-roster H-score if we buy at the Preparation/static price.
    #[serde(default)]
    pub prep_market_projected_matchup_win_probability: f64,
    /// Best completed-roster H-score if we buy at the opponent-aware competition price.
...
#[derive(Debug, Clone)]
pub struct FullBeamPriceCurve {
    pub reference_sale_price: u16,
    pub market_seconds: f64,
    pub pass_matchup_win_probability: f64,
    pub pass_expected_categories: f64,
    pub pass_build_name: String,
    pub pass_j_name: String,
    pub pass_seconds: f64,
    pub points: Vec<FullBeamPricePoint>,
}

/// Shared fixed-market completion library used by the live FULL Max Bid cache.
///
/// Instead of rerunning the beam search once per candidate price, BirdBoard
/// keeps a cost-diverse set of promising future roster states. Candidate bids
/// then become cheap lookups over feasible states. The feasible set only
/// shrinks as the candidate price rises, so the resulting BUY curve is
/// monotone by construction.
#[derive(Debug, Clone)]
pub struct FullPricingFrontier {
    roster_size_at_start: usize,
    own_budget_remaining: u16,
    opponent_sums: Vec<([f64; 9], usize)>,
    replacement_x_scores: [f64; 9],
    pass_entries: Vec<PricingFrontierEntry>,
    buy_entries: Vec<PricingFrontierEntry>,
    pub market_seconds: f64,
...
#[derive(Debug, Clone)]
struct PricingFrontierEntry {
    strategy_weights: [f64; 9],
    completed_x: [f64; 9],
    future_players: Vec<PlayerId>,
    intentional_spend: u16,
}

#[derive(Debug, Clone, Copy)]
pub struct FullPricingFrontierCoverage {
    pub pass_total: usize,
    pub pass_without_candidate: usize,
    pub pass_with_candidate: usize,
    pub buy_total: usize,
    pub buy_without_candidate: usize,
    pub buy_with_candidate: usize,
}

#[derive(Debug, Clone)]
pub struct CandidatePricingFrontier {
    candidate: PlayerId,
    roster_size_at_start: usize,
    own_budget_remaining: u16,
    opponent_sums: Vec<([f64; 9], usize)>,
    replacement_x_scores: [f64; 9],
    pass_evaluation: DynamicStateEvaluation,
    buy_entries: Vec<PricingFrontierEntry>,
    pub reference_sale_price: u16,
    pub market_seconds: f64,
    pub pass_beam_seconds: f64,
    pub buy_beam_seconds: f64,
    pub pass_strategy_count: usize,
    pub buy_strategy_count: usize,
}

#[derive(Debug, Clone)]
pub struct FullPricingBidAdvice {
    pub fair_price: u16,
    pub expected_edge: i16,
    pub pass_projected_matchup_win_probability: f64,
    pub prep_market_projected_matchup_win_probability: f64,
    pub competition_projected_matchup_win_probability: f64,
    pub fair_price_projected_matchup_win_probability: f64,
    pub prep_market_price: u16,
    pub evaluated_prep_market_price: u16,
    pub competition_price: u16,
    pub evaluated_competition_price: u16,
    pub build_name: String,
    pub j_name: String,
    pub frontier_market_seconds: f64,
    pub frontier_beam_seconds: f64,
}
...
pub struct DurantModel {
    ...
    /// Optional independently simulated preseason auction-equilibrium curve.
    /// When present, every internal market construction uses this curve as the
    /// relative demand prior and re-scales it to the CURRENT room's dollars.
    /// This means Live board, Strategy, competition pricing, and Max Bid all
    /// share the same independently generated market economics.
    equilibrium_prices: Option<HashMap<PlayerId, f64>>,
    /// Optional ABSOLUTE market-price prior used by validation harnesses.
    /// Unlike `equilibrium_prices`, these dollar values are not normalized to
    /// consume the full room budget.  They remain anchored to their original
    /// dollar scale and only receive relative live-room inflation/deflation.
    /// Production BirdBoard does not set this field.
    absolute_market_prices: Option<HashMap<PlayerId, f64>>,
    /// Baseline ratio between room discretionary cash and the absolute prior's
    /// draftable price mass.  Dividing the live ratio by this value guarantees
    /// an inflation factor of exactly 1.0 at the validation room's initial
    /// state, even when the validation candidate pool is intentionally
    /// truncated for historical-week scoring.
    absolute_market_reference_ratio: Option<f64>,
    /// Empirical probability calibration fitted on historical ACTIVE-WEEK holdout data.
    /// Identity (slope=1) when no calibration artifact has been fitted yet.
    h_calibration: HCalibration,
...
impl DurantModel {
    pub fn empty(league_teams: usize, roster_size: usize) -> Self {
        ...
        Self {
            ...
            x_variance: [0.0; 9],
            scores: Vec::new(),
            equilibrium_prices: None,
            absolute_market_prices: None,
            absolute_market_reference_ratio: None,
            h_calibration: HCalibration::default(),
    ...
    pub fn from_stats(
        ...
        Self::from_stats_internal(stats, league_teams, roster_size, true)
    }

    /// Historical-only constructor used by the H calibration harness. It uses
    /// exactly the same DURANT fitting/scoring machinery as runtime, but it
    /// deliberately ignores present-day manual projection overrides so future
    /// beliefs cannot leak into a historical holdout experiment.
    pub(crate) fn from_historical_stats_for_validation(
        stats: &StatsBundle,
        league_teams: usize,
        roster_size: usize,
    ) -> Result<Self> {
        Self::from_stats_internal(stats, league_teams, roster_size, false)
    }

    fn from_stats_internal(
        stats: &StatsBundle,
        league_teams: usize,
        roster_size: usize,
        use_manual_overrides: bool,
    ) -> Result<Self> {
        if league_teams == 0 {
        ...
        let mut manual_overrides = if use_manual_overrides {
            load_manual_projection_overrides(MANUAL_OVERRIDES_PATH)?
        } else {
            Vec::new()
        };
        ...
        // Q is a historical reference population. Preparation edits must never
        // change who belongs to it: manual projections describe our beliefs
        // about NEXT season, while Q exists only to fit the historical
        // environment (mu/sigma/tau and X variance). Therefore eligibility is
        // determined solely from historical weekly coverage.
        ...
        if eligible_ids.len() < reference_size {
            bail!(
                "only {} historical players have at least {} active weeks; Durant needs {} for Q",
                eligible_ids.len(),
                MIN_ACTIVE_WEEKS,
                reference_size
            );
        ...
        let historical_scores = stats
            .players
            .iter()
            .map(|player| score_stat_projection(player, games_per_active_week, parameters))
            .collect::<Vec<_>>();

        // X variance is another property of the historical reference
        // environment, so fit it BEFORE applying any preparation overrides.
        // This keeps every user edit from moving the baseline faced by every
        // other player.
        let x_variance = fit_x_variance(&reference_players, &historical_scores, parameters);

        let mut scores = historical_scores;

        // Manual projections replace only the edited player's final projected
        // mean line. They cannot change Q, mu/sigma/tau, active-week volume, or
        // X variance.
        ...
        let h_calibration = if use_manual_overrides {
            load_h_calibration(&stats.cache_dir)
        } else {
            HCalibration::default()
        };
        let model_fingerprint = fingerprint_model(
            &scores,
            parameters,
            reference_size,
            roster_size,
    ...
    pub fn set_equilibrium_prices(&mut self, prices: Option<HashMap<PlayerId, f64>>) {
        if let Some(active) = prices.as_ref().filter(|prices| !prices.is_empty()) {
            // Pricing changes must invalidate every persistent dynamic/auction
            // cache just like a projection change would. Hash the independent
            // market into the existing model fingerprint deterministically.
            let mut hasher = DefaultHasher::new();
            self.model_fingerprint.hash(&mut hasher);
            "independent_equilibrium_market_v1".hash(&mut hasher);
            let mut rows = active.iter().collect::<Vec<_>>();
            rows.sort_by(|left, right| left.0.0.cmp(&right.0.0));
            for (player_id, price) in rows {
                player_id.hash(&mut hasher);
                price.to_bits().hash(&mut hasher);
            }
            self.model_fingerprint = hasher.finish();
        }
        self.absolute_market_prices = None;
        self.absolute_market_reference_ratio = None;
        self.equilibrium_prices = prices;
    }

    /// Install a dollar-denominated market prior without turning it into a
    /// relative demand curve.  This exists for external-market validation
    /// (currently ESPN): a published $74 value must begin as $74, not be
    /// inflated merely because the historical backtest pool omits players.
    ///
    /// `prices` should already be restricted to the validation candidate pool.
    /// The baseline ratio is computed once from that exact pool so subsequent
    /// room inflation measures only RELATIVE over/underspending versus the
    /// starting external market.
    pub fn set_absolute_market_prices(
        &mut self,
        prices: Option<HashMap<PlayerId, f64>>,
        config: AuctionConfig,
    ) {
        let active = prices.as_ref().filter(|prices| !prices.is_empty());
        let reference_ratio = active.and_then(|active| {
            let mut excess = active
                .values()
                .copied()
                .map(|price| (price - config.minimum_bid as f64).max(0.0))
                .collect::<Vec<_>>();
            excess.sort_by(|left, right| right.total_cmp(left));
            let slots = self.reference_size.min(excess.len());
            let value_mass = excess.into_iter().take(slots).sum::<f64>();
            if value_mass <= f64::EPSILON {
                None
            } else {
                let league_teams = self.league_teams().max(1);
                let initial_cash = league_teams as f64 * config.starting_budget as f64;
                let reserve = slots as f64 * config.minimum_bid as f64;
                Some((initial_cash - reserve).max(0.0) / value_mass)
            }
        });
        ...
        if let Some(active) = active {
            let mut hasher = DefaultHasher::new();
            self.model_fingerprint.hash(&mut hasher);
            "absolute_external_market_v1".hash(&mut hasher);
            let mut rows = active.iter().collect::<Vec<_>>();
            rows.sort_by(|left, right| left.0.0.cmp(&right.0.0));
            for (player_id, price) in rows {
                player_id.hash(&mut hasher);
            ...
            if let Some(ratio) = reference_ratio {
        ...
        self.absolute_market_prices = prices;
        self.absolute_market_reference_ratio = reference_ratio;
    }

    pub fn uses_equilibrium_market(&self) -> bool {
        self.equilibrium_prices
            .as_ref()
            .is_some_and(|prices| !prices.is_empty())
    }

    pub(crate) fn h_calibration_slope(&self) -> f64 {
        self.h_calibration.slope
    }
    ...
    /// Market board driven by BirdBoard's independently simulated auction
    /// equilibrium rather than the legacy linear G/$ curve. The saved
    /// preseason prices are used only as RELATIVE demand weights; the current
    /// room's remaining dollars are redistributed over the remaining player
    /// pool every time the real draft state changes. This keeps the market
    /// endogenous and budget-conserving without fitting any ESPN values.
    pub fn market_board_with_equilibrium_prices(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
        opponent_budgets_remaining: &[u16],
        candidates: &[PlayerId],
        equilibrium_prices: &HashMap<PlayerId, f64>,
        config: AuctionConfig,
    ) -> MarketBoard {
        let totals = self.auction_market_totals(
            own_roster,
        ...
        let economy = self.estimate_market_economy_from_equilibrium_prices(
            candidates,
            None,
            totals,
            equilibrium_prices,
    ...
    /// Predict one complete 13-player-vs-13-player weekly matchup using the
    /// exact current H core. This is intentionally crate-private: it exists so
    /// the historical calibration harness can test H without reimplementing
    /// the formula in a second module.
    pub(crate) fn validation_complete_matchup_prediction(
        &self,
        own_roster: &[PlayerId],
        opponent_roster: &[PlayerId],
    ) -> Option<([f64; 9], f64)> {
        if own_roster.len() != self.team_size || opponent_roster.len() != self.team_size {
            return None;
        }
        if own_roster
            .iter()
            .chain(opponent_roster.iter())
            .any(|id| self.x_score_for(id).is_none())
        {
            return None;
        }

        let own_x = self.aggregate_x_scores(own_roster);
        let opponent_x = self.aggregate_x_scores(opponent_roster);
        let evaluation =
            self.evaluate_dynamic_state_raw(own_x, &[(opponent_x, opponent_roster.len())]);

        Some((
            evaluation.category_win_probabilities,
            evaluation.matchup_win_probability,
        ))
    }
    ...
    /// Offline benchmark for the Strategy pricing funnel.
    ///
    /// The supplied `search_strategy_weights` are the CURRENT stage-aware j
    /// vocabulary (EARLY-global or MID-local).  The compact
    /// `pricing_strategy_weights` are held fixed for every experiment.
    ///
    /// For each K we:
    ///   1. rank the ENTIRE active j bank under the cheap static market once,
    ///   2. take the top-K coarse plans and union the players they target,
    ///   3. rationally reprice only that focused player set,
    ///   4. rerun the ENTIRE active j bank under the focused market.
    ///
    /// The reference uses the exact same compact pricing vocabulary and the
    /// same 16-strategy future-pricing limit, but rationally reprices EVERY
    /// remaining candidate.  Thus the reported delta isolates the player-focus
    /// funnel rather than mixing in j-bank or pricing-core differences.
    /// Full-focus runtime Strategy planner.
    ///
    /// This is the accurate reference market used by the v42 funnel benchmark:
    /// every remaining candidate is rationally repriced with the compact
    /// opponent-pricing strategy bank, then ALL active stage-aware search j's
    /// are evaluated against that one fixed opponent-aware market.
    ///
    /// It is intentionally expensive and is meant to run off the UI thread.
    pub fn roster_plan_full_rational_with_strategy_weights(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
        ...
        // Full focus: every remaining player gets an opponent-specific rational
        // market price before our stage-aware strategy search is evaluated.
        let economy = self.focused_rational_opponent_market_economy(
            opponent_rosters,
            opponent_budgets_remaining,
            candidates,
            candidates,
        ...
        // Full-focus Strategy gets the expensive roster-combination search.
        // Market construction above is unchanged: the only difference from
        // the v44 planner is that each j may keep several plausible partial
        // rosters instead of committing to its first affordable ranked path.
        let rollouts = self.top_intentional_auction_rollouts_beam(
            own_roster.len(),
            own_sum,
            own_open_slots,
            own_budget_remaining,
        ...
        self.validation_plan_from_rollouts(current, rollouts)
    }

    /// Controlled full-beam price curve used to validate Max Bid semantics.
    ///
    /// The future market is solved ONCE after the nominated player leaves the
    /// board at `reference_sale_price`, then frozen for every tested BUY price.
    /// Only our remaining budget changes across points. This deliberately
    /// isolates the roster-combination optimizer from endogenous market-price
    /// feedback: with a fixed future market, a cheaper purchase should never
    /// make the best attainable H worse except for tiny beam-search noise.
    pub fn full_beam_price_curve_fixed_market(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
        ...
        candidate: &PlayerId,
        prices: &[u16],
        pass_strategy_weights: &[[f64; 9]],
        buy_strategy_weights: &[[f64; 9]],
        pricing_strategy_weights: &[[f64; 9]],
        reference_sale_price: u16,
        config: AuctionConfig,
    ) -> Option<FullBeamPriceCurve> {
        if own_roster.len() >= self.team_size
            || pass_strategy_weights.is_empty()
            || buy_strategy_weights.is_empty()
            || pricing_strategy_weights.is_empty()
            || own_roster.iter().any(|player_id| player_id == candidate)
        {
            return None;
        }
        ...
        let mut future_candidates = Vec::<PlayerId>::new();
        for player_id in candidates {
            if player_id == candidate
                || own_roster.iter().any(|owned| owned == player_id)
                || self.score_for(player_id).is_none()
            {
                continue;
            }
            if seen.insert(player_id.clone()) {
                future_candidates.push(player_id.clone());
            }
        }

        let current_market_totals = self.auction_market_totals(
            own_roster,
            own_budget_remaining,
            opponent_rosters,
            opponent_budgets_remaining,
        ...
        let post_sale_totals = AuctionMarketTotals {
            remaining_slots: current_market_totals.remaining_slots.saturating_sub(1),
            remaining_dollars: current_market_totals
                .remaining_dollars
                .saturating_sub(reference_sale_price as u32),
            max_legal_market_bid: current_market_totals.max_legal_market_bid,
        };
        ...
        let pricing_strategies = pricing_strategy_weights
            ...
            .map(|weights| RolloutStrategy { weights })
            .collect::<Vec<_>>();
        let pricing_rankings =
            self.rank_available_by_strategy(&future_candidates, &pricing_strategies);

        let market_started = std::time::Instant::now();
        let economy = self.focused_rational_opponent_market_economy(
            opponent_rosters,
            opponent_budgets_remaining,
            &future_candidates,
        ...
        let market_seconds = market_started.elapsed().as_secs_f64();

        let own_sum = self.aggregate_x_scores(own_roster);
        let own_open_slots = self.team_size.saturating_sub(own_roster.len());

        let pass_strategies = pass_strategy_weights
            .iter()
            .copied()
            .map(|weights| RolloutStrategy { weights })
            .collect::<Vec<_>>();
        let pass_rankings = self.rank_available_by_strategy(&future_candidates, &pass_strategies);
        let pass_started = std::time::Instant::now();
        let mut pass_rollouts = self.top_intentional_auction_rollouts_beam(
            own_roster.len(),
            own_sum,
            own_open_slots,
            own_budget_remaining,
        ...
        sort_and_truncate_auction_rollouts(&mut pass_rollouts, 1);
        let pass = pass_rollouts.into_iter().next()?;
        let pass_seconds = pass_started.elapsed().as_secs_f64();

        let buy_strategies = buy_strategy_weights
            .iter()
            .copied()
            .map(|weights| RolloutStrategy { weights })
            .collect::<Vec<_>>();
        let buy_rankings = self.rank_available_by_strategy(&future_candidates, &buy_strategies);
        let buy_sum = add_vectors(own_sum, candidate_x);
        let buy_roster_size = own_roster.len().saturating_add(1);
        let buy_future_slots = own_open_slots.saturating_sub(1);
        let max_bid = max_legal_bid(own_budget_remaining, own_open_slots, config.minimum_bid);

        let mut unique_prices = prices
            .iter()
            .copied()
            .filter(|price| *price >= config.minimum_bid && *price <= max_bid)
            .collect::<Vec<_>>();
        unique_prices.sort_unstable();
        unique_prices.dedup();

        let mut points = Vec::<FullBeamPricePoint>::with_capacity(unique_prices.len());
        for price in unique_prices {
            let started = std::time::Instant::now();
            let mut rollouts = self.top_intentional_auction_rollouts_beam(
                buy_roster_size,
                buy_sum,
                buy_future_slots,
                own_budget_remaining.saturating_sub(price),
                &buy_strategies,
                &buy_rankings,
                &economy,
                &opponent_sums,
            ...
            sort_and_truncate_auction_rollouts(&mut rollouts, 1);
            let best = rollouts.into_iter().next()?;
            points.push(FullBeamPricePoint {
                price,
                matchup_win_probability: best.evaluation.matchup_win_probability,
                expected_categories: best.evaluation.expected_categories,
                build_name: describe_projected_build(best.evaluation.category_win_probabilities),
                j_name: describe_strategy(
                    &best.strategy,
                    best.evaluation.category_win_probabilities,
        ...
        Some(FullBeamPriceCurve {
            reference_sale_price,
            market_seconds,
            pass_matchup_win_probability: pass.evaluation.matchup_win_probability,
            pass_expected_categories: pass.evaluation.expected_categories,
            pass_build_name: describe_projected_build(pass.evaluation.category_win_probabilities),
            pass_j_name: describe_strategy(
                &pass.strategy,
                pass.evaluation.category_win_probabilities,
            ),
            pass_seconds,
            points,
        })
    ...
    /// Candidate-specific Strategy budget frontier for live Max Bid.
    ///
    /// This keeps the model semantics that made the Strategy screen useful:
    /// the nominated player is removed from the future market, opponents are
    /// rationally repriced in that candidate-specific world, PASS is solved by
    /// the normal full beam, and BUY locks the candidate onto our roster before
    /// the H-aware beam search begins.  The BUY beam retains spend-diverse
    /// completions so one expensive search supplies the whole price curve.
    pub fn build_candidate_pricing_frontier_with_strategy_weights(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
        ...
        config: AuctionConfig,
        cancel: Option<&AtomicBool>,
    ) -> Option<CandidatePricingFrontier> {
        if own_roster.len() >= self.team_size
            || own_roster.iter().any(|player_id| player_id == candidate)
            || pass_strategy_weights.is_empty()
            || buy_strategy_weights.is_empty()
        ...
        // PASS is one ordinary Strategy-quality solve in the same
        // candidate-specific future market.
        let pass_strategies = pass_strategy_weights
            .iter()
            .copied()
            .map(|weights| RolloutStrategy { weights })
        ...
        sort_and_truncate_auction_rollouts(&mut pass_rollouts, 1);
        let pass = pass_rollouts.into_iter().next()?;
        let pass_beam_seconds = pass_started.elapsed().as_secs_f64();
        if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
            return None;
        }

        // BUY locks the candidate onto our roster before beam pruning.  This is
        // the crucial difference from the old shared frontier: the candidate's
        // category shape influences every preview and every retained branch.
        let buy_roster_size = own_roster.len().saturating_add(1);
        let buy_future_slots = self.team_size.saturating_sub(buy_roster_size);
        let intentional_target = INTENTIONAL_ROSTER_SLOTS.min(self.team_size);
        let buy_depth = intentional_target
            .saturating_sub(buy_roster_size)
            .min(buy_future_slots);
        let buy_sum = add_vectors(own_sum, candidate_x);
        let max_future_budget = own_budget_remaining.saturating_sub(config.minimum_bid);
        ...
        #[derive(Clone)]
        struct BudgetBeamState {
            completed_x: [f64; 9],
            budget: u16,
            intentional_spend: u16,
            future_players: Vec<PlayerId>,
            next_rank_index: usize,
        }

        #[derive(Clone)]
        struct ScoredBudgetState {
            state: BudgetBeamState,
            preview: DynamicStateEvaluation,
            preview_spend: u16,
        }

        const BUDGET_FRONTIER_BEAM_WIDTH: usize = 24;
        const BRANCHES_PER_STATE: usize = 24;
        const SPEND_BUCKET_DOLLARS: u16 = 4;

        let buy_started = std::time::Instant::now();
        let per_strategy = buy_strategies
            .par_iter()
            .zip(buy_rankings.par_iter())
            .map(|(strategy, ranked_players)| {
                if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
                    return Vec::<PricingFrontierEntry>::new();
                }

                let complete_preview = |partial: &BudgetBeamState| {
                    let mut state = partial.clone();
                    let mut cursor = state.next_rank_index;
                    let mut intentional_index = state.future_players.len();

                    while intentional_index < buy_depth {
                        let slots_after_this_purchase =
                            buy_future_slots.saturating_sub(intentional_index + 1);
                        let reserve_after = (slots_after_this_purchase as u32)
                            .saturating_mul(config.minimum_bid as u32);
                        let max_spend = (state.budget as u32)
                            .saturating_sub(reserve_after)
                            .min(u16::MAX as u32) as u16;

                        let mut chosen = None;
                        while cursor < ranked_players.len() {
                            let rank_index = cursor;
                            ...
                            let price = economy
                                ...
                                .unwrap_or(config.minimum_bid);
                            if price <= max_spend {
                                chosen = Some((rank_index, player_id, x, price));
                                break;
                            }
                        }

                        let Some((rank_index, player_id, x, price)) = chosen else {
                            break;
                        };
                        state.completed_x = add_vectors(state.completed_x, x);
                        state.budget = state.budget.saturating_sub(price);
                        state.intentional_spend = state.intentional_spend.saturating_add(price);
                        state.future_players.push(player_id.clone());
                        state.next_rank_index = rank_index + 1;
                        intentional_index += 1;
                    }

                    let flier_count = buy_future_slots.saturating_sub(state.future_players.len());
                    let mut preview_x = state.completed_x;
                    let mut preview_spend = state.intentional_spend;
                    for _ in 0..flier_count {
                        if state.budget < config.minimum_bid {
                            break;
                        }
                        preview_x = add_vectors(preview_x, economy.snapshot.replacement_x_scores);
                        state.budget = state.budget.saturating_sub(config.minimum_bid);
                        preview_spend = preview_spend.saturating_add(config.minimum_bid);
                    }
                    (
                        self.evaluate_dynamic_state(preview_x, &opponent_sums),
                        preview_spend,
                    )
                };

                let mut beam = vec![BudgetBeamState {
                    completed_x: buy_sum,
                    budget: max_future_budget,
                    intentional_spend: 0,
                    future_players: Vec::with_capacity(buy_depth),
                    next_rank_index: 0,
                }];

                for intentional_index in 0..buy_depth {
                    if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
                        return Vec::<PricingFrontierEntry>::new();
                    }
                    let slots_after_this_purchase =
                        buy_future_slots.saturating_sub(intentional_index + 1);
                    let mut children = Vec::<ScoredBudgetState>::with_capacity(
                        beam.len().saturating_mul(BRANCHES_PER_STATE),
                    );
                    ...
                    for state in &beam {
                        ...
                        let max_spend = (state.budget as u32)
                            .saturating_sub(reserve_after)
                            .min(u16::MAX as u32) as u16;
                        // The ordinary Strategy beam branches over the first
                        // 24 j-ranked affordable players. A budget frontier also
                        // needs cheap alternatives that may only matter after an
                        // expensive nomination. Keep half the branches rank-first
                        // and use the other half for price-bucket diversity.
                        const RANK_FIRST_BRANCHES: usize = BRANCHES_PER_STATE / 2;
                        const PLAYER_PRICE_BUCKET: u16 = 5;
                        let mut branch_indices = Vec::<usize>::with_capacity(BRANCHES_PER_STATE);
                        let mut chosen_indices = HashSet::<usize>::new();

                        for rank_index in state.next_rank_index..ranked_players.len() {
                            if branch_indices.len() >= RANK_FIRST_BRANCHES {
                                break;
                            }
                            let player_id = &ranked_players[rank_index];
                            let price = economy
                                .prices
                                .get(player_id)
                                .copied()
                                .unwrap_or(config.minimum_bid);
                            if price <= max_spend && self.x_score_for(player_id).is_some() {
                                branch_indices.push(rank_index);
                                chosen_indices.insert(rank_index);
                            }
                        }

                        let mut seen_price_buckets = HashSet::<u16>::new();
                        for rank_index in state.next_rank_index..ranked_players.len() {
                            if branch_indices.len() >= BRANCHES_PER_STATE {
                                break;
                            }
                            if chosen_indices.contains(&rank_index) {
                                continue;
                            }
                            let player_id = &ranked_players[rank_index];
                            let price = economy
                                ...
                                .copied()
                                .unwrap_or(config.minimum_bid);
                            if price > max_spend || self.x_score_for(player_id).is_none() {
                                continue;
                            }
                            let bucket = price / PLAYER_PRICE_BUCKET;
                            if seen_price_buckets.insert(bucket) {
                                branch_indices.push(rank_index);
                                chosen_indices.insert(rank_index);
                            }
                        }
                        ...
                        for rank_index in branch_indices {
                            let player_id = &ranked_players[rank_index];
                            let Some(x) = self.x_score_for(player_id) else {
                                continue;
                            };
                            ...
                            child.completed_x = add_vectors(child.completed_x, x);
                            child.budget = child.budget.saturating_sub(price);
                            child.intentional_spend = child.intentional_spend.saturating_add(price);
                            child.future_players.push(player_id.clone());
                            child.next_rank_index = rank_index + 1;
                            let (preview, preview_spend) = complete_preview(&child);
                            children.push(ScoredBudgetState {
                                state: child,
                                preview,
                                preview_spend,
                            });
                    ...
                    children.sort_by(|a, b| {
                        b.preview
                            .matchup_win_probability
                            .total_cmp(&a.preview.matchup_win_probability)
                            .then_with(|| {
                                b.preview
                                    .expected_categories
                                    .total_cmp(&a.preview.expected_categories)
                            })
                            .then_with(|| a.preview_spend.cmp(&b.preview_spend))
                    });

                    // Preserve the best H preview across many spend levels.
                    // This is a candidate-specific budget frontier, so low-cost
                    // paths remain available when a higher nomination price
                    // leaves us less money later.
                    let mut bucket_seen = HashSet::<u16>::new();
                    let mut bucket_best = Vec::<usize>::new();
                    for (index, child) in children.iter().enumerate() {
                        let bucket = child.preview_spend / SPEND_BUCKET_DOLLARS;
                        if bucket_seen.insert(bucket) {
                            bucket_best.push(index);
                        }
                    }
                    bucket_best.sort_by_key(|index| children[*index].preview_spend);

                    let mut pareto_indices = Vec::<usize>::new();
                    let mut best_h = f64::NEG_INFINITY;
                    for index in bucket_best {
                        let h = children[index].preview.matchup_win_probability;
                        if h > best_h + 1.0e-9 {
                            pareto_indices.push(index);
                            best_h = h;
                        }
                    }

                    let mut chosen_indices =
                        Vec::<usize>::with_capacity(BUDGET_FRONTIER_BEAM_WIDTH);
                    if pareto_indices.len() > BUDGET_FRONTIER_BEAM_WIDTH {
                        for slot in 0..BUDGET_FRONTIER_BEAM_WIDTH {
                            let position = slot * (pareto_indices.len() - 1)
                                / (BUDGET_FRONTIER_BEAM_WIDTH - 1);
                            chosen_indices.push(pareto_indices[position]);
                        }
                    } else {
                        chosen_indices.extend(pareto_indices);
                    }
                    let mut chosen_set = chosen_indices.iter().copied().collect::<HashSet<_>>();
                    if chosen_indices.len() < BUDGET_FRONTIER_BEAM_WIDTH {
                        for index in 0..children.len() {
                            if chosen_indices.len() >= BUDGET_FRONTIER_BEAM_WIDTH {
                                break;
                            }
                            if chosen_set.insert(index) {
                    ...
                    beam = chosen_indices
                        .into_iter()
                        .map(|index| children[index].state.clone())
                        .collect();
                }

                beam.into_iter()
                    .map(|state| PricingFrontierEntry {
                        strategy_weights: strategy.weights,
                        completed_x: state.completed_x,
                        future_players: state.future_players,
                        intentional_spend: state.intentional_spend,
                    })
                    .collect::<Vec<_>>()
            })
            .collect::<Vec<_>>();
        ...
        let mut buy_entries = Vec::<PricingFrontierEntry>::new();
        for mut entries in per_strategy {
            buy_entries.append(&mut entries);
        }
        if buy_entries.is_empty() {
            return None;
        }

        Some(CandidatePricingFrontier {
            candidate: candidate.clone(),
            roster_size_at_start: own_roster.len(),
            own_budget_remaining,
            opponent_sums,
            replacement_x_scores: economy.snapshot.replacement_x_scores,
            pass_evaluation: pass.evaluation,
            buy_entries,
            reference_sale_price,
            market_seconds,
            pass_beam_seconds,
            buy_beam_seconds: buy_started.elapsed().as_secs_f64(),
            pass_strategy_count: pass_strategy_weights.len(),
            buy_strategy_count: buy_strategy_weights.len(),
        })
    }

    /// Convert one candidate-specific Strategy frontier into a live Max Bid.
    /// The future market is fixed at the expected sale price, while our budget
    /// varies dollar-by-dollar. Because feasibility only shrinks as price rises,
    /// the resulting BUY envelope is monotone by construction.
    pub fn candidate_pricing_bid_from_frontier(
        &self,
        frontier: &CandidatePricingFrontier,
        candidate: &PlayerId,
        prep_market_price: u16,
        competition_price: u16,
        config: AuctionConfig,
    ) -> Option<FullPricingBidAdvice> {
        if &frontier.candidate != candidate || frontier.roster_size_at_start >= self.team_size {
            return None;
        }

        let open_slots = self.team_size.saturating_sub(frontier.roster_size_at_start);
        let max_bid = max_legal_bid(
            frontier.own_budget_remaining,
            open_slots,
            config.minimum_bid,
        );
        ...
        struct EvaluatedChoice {
            ...
            total_future_spend: u16,
            strategy_weights: [f64; 9],
        }

        let better = |left: EvaluatedChoice, right: EvaluatedChoice| {
            left.evaluation
                .matchup_win_probability
                .total_cmp(&right.evaluation.matchup_win_probability)
                .then_with(|| {
                    left.evaluation
                        .expected_categories
                        .total_cmp(&right.evaluation.expected_categories)
                })
                .then_with(|| right.total_future_spend.cmp(&left.total_future_spend))
                .is_gt()
        };

        let buy_future_slots = open_slots.saturating_sub(1);
        let mut buy_choices = Vec::<EvaluatedChoice>::with_capacity(frontier.buy_entries.len());
        for entry in &frontier.buy_entries {
            let flier_count = buy_future_slots.saturating_sub(entry.future_players.len());
            let flier_spend = (flier_count as u32)
                .saturating_mul(config.minimum_bid as u32)
                .min(u16::MAX as u32) as u16;
            let total_spend = entry.intentional_spend.saturating_add(flier_spend);
            let mut final_x = entry.completed_x;
            for _ in 0..flier_count {
                final_x = add_vectors(final_x, frontier.replacement_x_scores);
            }
            buy_choices.push(EvaluatedChoice {
                evaluation: self.evaluate_dynamic_state(final_x, &frontier.opponent_sums),
                total_future_spend: total_spend,
                strategy_weights: entry.strategy_weights,
            });
        }
        if buy_choices.is_empty() {
            return None;
        }

        let best_at_price = |price: u16| -> Option<EvaluatedChoice> {
            let future_budget = frontier.own_budget_remaining.saturating_sub(price);
            let mut best = None::<EvaluatedChoice>;
            for choice in &buy_choices {
                if choice.total_future_spend > future_budget {
                    continue;
                }
                if match best {
                    None => true,
                    Some(current) => better(*choice, current),
                } {
                    best = Some(*choice);
                }
            }
        ...
        let evaluated_prep_market_price = prep_market_price.max(config.minimum_bid).min(max_bid);
        let evaluated_competition_price = competition_price.max(config.minimum_bid).min(max_bid);
        let prep_choice = best_at_price(evaluated_prep_market_price)?;
        let competition_choice = best_at_price(evaluated_competition_price)?;

        let pass_h = frontier.pass_evaluation.matchup_win_probability;
        let mut fair_price = 0u16;
        let mut fair_choice = None::<EvaluatedChoice>;
        for price in config.minimum_bid..=max_bid {
            let Some(choice) = best_at_price(price) else {
                continue;
            };
            if choice.evaluation.matchup_win_probability + H_INDIFFERENCE_EPSILON >= pass_h {
                fair_price = price;
                fair_choice = Some(choice);
            }
        }
        let fair_choice = fair_choice.or_else(|| best_at_price(config.minimum_bid));
        let fair_h = fair_choice
            .map(|choice| choice.evaluation.matchup_win_probability)
            .unwrap_or(0.0);
        let label_choice = fair_choice.unwrap_or(competition_choice);
        let strategy = RolloutStrategy {
            weights: label_choice.strategy_weights,
        };

        Some(FullPricingBidAdvice {
            fair_price,
            expected_edge: fair_price as i16 - competition_price as i16,
            pass_projected_matchup_win_probability: pass_h,
            prep_market_projected_matchup_win_probability: prep_choice
                .evaluation
                .matchup_win_probability,
            competition_projected_matchup_win_probability: competition_choice
                .evaluation
                .matchup_win_probability,
            fair_price_projected_matchup_win_probability: fair_h,
            prep_market_price,
            evaluated_prep_market_price,
            competition_price,
            evaluated_competition_price,
            build_name: describe_projected_build(
                label_choice.evaluation.category_win_probabilities,
            ),
            j_name: describe_strategy(
                &strategy,
                label_choice.evaluation.category_win_probabilities,
            ),
            frontier_market_seconds: frontier.market_seconds,
            frontier_beam_seconds: frontier.pass_beam_seconds + frontier.buy_beam_seconds,
        })
    }

    /// Build one cost-diverse completion frontier for the current draft state.
    ///
    /// This is the live pricing counterpart to the v52 diagnostic. The expensive
    /// opponent market and beam search run once in the background. We retain
    /// promising partial rosters across several spend levels rather than only
    /// the single highest-H completion, which lets every candidate price reuse
    /// the same search work.
    pub fn build_full_pricing_frontier_with_strategy_weights(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
        ...
        let future_slots = self.team_size.saturating_sub(own_roster.len());
        let intentional_target = INTENTIONAL_ROSTER_SLOTS.min(self.team_size);
        let pass_depth = intentional_target
            .saturating_sub(own_roster.len())
            .min(future_slots);
        let buy_roster_size = own_roster.len().saturating_add(1);
        let buy_future_slots = self.team_size.saturating_sub(buy_roster_size);
        let buy_depth = intentional_target
        ...
        struct ScoredFrontierState {
            state: FrontierBeamState,
            preview: DynamicStateEvaluation,
            preview_spend: u16,
        }

        const FRONTIER_BEAM_WIDTH: usize = 16;
        const BRANCHES_PER_STATE: usize = 24;
        const SPEND_BUCKET_DOLLARS: u16 = 5;

        let beam_started = std::time::Instant::now();
        let per_strategy = search_strategies
            .par_iter()
            .zip(search_rankings.par_iter())
            .map(|(strategy, ranked_players)| {
                if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
                    return (
                        Vec::<PricingFrontierEntry>::new(),
                        Vec::<PricingFrontierEntry>::new(),
                    );
                }

                let capture = |beam: &[FrontierBeamState]| {
                    beam.iter()
                        .map(|state| PricingFrontierEntry {
                            strategy_weights: strategy.weights,
                            completed_x: state.completed_x,
                            future_players: state.future_players.clone(),
                            intentional_spend: state.intentional_spend,
                        })
                        .collect::<Vec<_>>()
                };

                let complete_preview = |partial: &FrontierBeamState| {
                    let mut state = partial.clone();
                    let mut cursor = state.next_rank_index;
                    let mut intentional_index = state.future_players.len();

                    while intentional_index < pass_depth {
                        let slots_after_this_purchase =
                            future_slots.saturating_sub(intentional_index + 1);
                        let reserve_after = (slots_after_this_purchase as u32)
                            .saturating_mul(config.minimum_bid as u32);
                        let max_spend = (state.budget as u32)
                            .saturating_sub(reserve_after)
                    ...
                    let flier_count = future_slots.saturating_sub(state.future_players.len());
                    let mut preview_x = state.completed_x;
                    let mut preview_spend = state.intentional_spend;
                    for _ in 0..flier_count {
                        if state.budget < config.minimum_bid {
                ...
                let mut beam = vec![FrontierBeamState {
                    completed_x: own_sum,
                    budget: own_budget_remaining,
                    intentional_spend: 0,
                    future_players: Vec::with_capacity(pass_depth),
                    next_rank_index: 0,
                }];
                let mut pass_entries = if pass_depth == 0 {
                    capture(&beam)
                } else {
                    Vec::new()
                };
                let mut buy_entries = if buy_depth == 0 {
                    capture(&beam)
                } else {
                    Vec::new()
                };

                for intentional_index in 0..pass_depth {
                    if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
                        break;
                    }
                    let slots_after_this_purchase =
                        future_slots.saturating_sub(intentional_index + 1);
                    let mut children = Vec::<ScoredFrontierState>::with_capacity(
                        beam.len().saturating_mul(BRANCHES_PER_STATE),
                    );
                    ...
                    for state in &beam {
                        ...
                        for rank_index in state.next_rank_index..ranked_players.len() {
                            if branches >= BRANCHES_PER_STATE {
                                break;
                            }
                            let player_id = &ranked_players[rank_index];
                            let Some(x) = self.x_score_for(player_id) else {
                            ...
                            child.next_rank_index = rank_index + 1;
                            let (preview, preview_spend) = complete_preview(&child);
                            children.push(ScoredFrontierState {
                                state: child,
                                preview,
                                preview_spend,
                            });
                    ...
                    // Keep the preview Pareto frontier over spend and H.
                    // `children` is already sorted best-H first, so the first
                    // child seen in each $5 spend bucket is that bucket's best
                    // preview. Walking those bucket winners cheap -> expensive
                    // and retaining only strict H improvements removes states
                    // that cost more without buying a better projected team.
                    let mut bucket_seen = HashSet::<u16>::new();
                    let mut bucket_best = Vec::<usize>::new();
                    for (index, child) in children.iter().enumerate() {
                        let bucket = child.preview_spend / SPEND_BUCKET_DOLLARS;
                    ...
                    // If the frontier itself is wider than the beam, sample it
                    // evenly from cheap to expensive so both ends survive.
                    let mut chosen_indices = Vec::<usize>::with_capacity(FRONTIER_BEAM_WIDTH);
                    if pareto_indices.len() > FRONTIER_BEAM_WIDTH {
                        for slot in 0..FRONTIER_BEAM_WIDTH {
                            let position =
                                slot * (pareto_indices.len() - 1) / (FRONTIER_BEAM_WIDTH - 1);
                            chosen_indices.push(pareto_indices[position]);
                        }
                    } else {
                        chosen_indices.extend(pareto_indices);
                    ...
                    // Spend/H Pareto states are mandatory. Any spare beam slots
                    // go to the strongest preview states overall, preserving the
                    // combination quality of the original 16x24 Strategy beam.
                    let mut chosen_set = chosen_indices.iter().copied().collect::<HashSet<_>>();
                    if chosen_indices.len() < FRONTIER_BEAM_WIDTH {
                        for index in 0..children.len() {
                            if chosen_indices.len() >= FRONTIER_BEAM_WIDTH {
                                break;
                            }
                    ...
                    let depth = intentional_index + 1;
                    if depth == buy_depth {
                        buy_entries = capture(&beam);
                    }
                    if depth == pass_depth {
                        pass_entries = capture(&beam);
                    }
                }

                (pass_entries, buy_entries)
            })
            .collect::<Vec<_>>();
        ...
        let mut pass_entries = Vec::<PricingFrontierEntry>::new();
        let mut buy_entries = Vec::<PricingFrontierEntry>::new();
        for (mut pass, mut buy) in per_strategy {
            pass_entries.append(&mut pass);
            buy_entries.append(&mut buy);
        }
        if pass_entries.is_empty() || buy_entries.is_empty() {
            return None;
        }

        Some(FullPricingFrontier {
            roster_size_at_start: own_roster.len(),
            own_budget_remaining,
            opponent_sums,
            replacement_x_scores: economy.snapshot.replacement_x_scores,
            ...
            buy_entries,
            market_seconds,
            beam_seconds: beam_started.elapsed().as_secs_f64(),
            strategy_count: search_strategy_weights.len(),
        })
    }

    /// Report how much of a shared frontier survives when `candidate` is
    /// removed. A low surviving fraction is a warning that the shared beam
    /// spent much of its search capacity on rosters containing that player.
    pub fn full_pricing_frontier_coverage(
        &self,
        frontier: &FullPricingFrontier,
        candidate: &PlayerId,
    ) -> FullPricingFrontierCoverage {
        let pass_with_candidate = frontier
            .pass_entries
            .iter()
            .filter(|entry| entry.future_players.iter().any(|id| id == candidate))
            .count();
        let buy_with_candidate = frontier
            .buy_entries
            .iter()
            .filter(|entry| entry.future_players.iter().any(|id| id == candidate))
            .count();
        FullPricingFrontierCoverage {
            pass_total: frontier.pass_entries.len(),
            pass_without_candidate: frontier
                .pass_entries
                .len()
                .saturating_sub(pass_with_candidate),
            pass_with_candidate,
            buy_total: frontier.buy_entries.len(),
            buy_without_candidate: frontier
                .buy_entries
                .len()
                .saturating_sub(buy_with_candidate),
            buy_with_candidate,
        }
    }

    /// Query a previously built completion frontier for one nominated player.
    /// This is intentionally cheap: no opponent repricing and no beam search.
    pub fn full_pricing_bid_from_frontier(
        &self,
        frontier: &FullPricingFrontier,
        candidate: &PlayerId,
        prep_market_price: u16,
        ...
        if frontier.roster_size_at_start >= self.team_size {
            return None;
        }
        let candidate_score = self.score_for(candidate)?;
        let candidate_x = score_to_x_vector(candidate_score, self.parameters);
        ...
        let add_replacements = |mut x: [f64; 9], count: usize| {
            for _ in 0..count {
                x = add_vectors(x, frontier.replacement_x_scores);
            }
            x
        };
        ...
        let pass_future_slots = open_slots;
        let mut pass_best: Option<EvaluatedChoice> = None;
        for entry in &frontier.pass_entries {
            if entry
                .future_players
                .iter()
                .any(|player_id| player_id == candidate)
            {
                continue;
            }
            let flier_count = pass_future_slots.saturating_sub(entry.future_players.len());
            let flier_spend = (flier_count as u32)
                .saturating_mul(config.minimum_bid as u32)
                .min(u16::MAX as u32) as u16;
            let total_spend = entry.intentional_spend.saturating_add(flier_spend);
            if total_spend > frontier.own_budget_remaining {
                continue;
            }
            let final_x = add_replacements(entry.completed_x, flier_count);
            let choice = EvaluatedChoice {
                evaluation: self.evaluate_dynamic_state(final_x, &frontier.opponent_sums),
                total_future_spend: total_spend,
                strategy_weights: entry.strategy_weights,
            };
            if match pass_best {
                ...
                pass_best = Some(choice);
            }
        }
        let pass_best = pass_best?;
        ...
        for entry in &frontier.buy_entries {
            ...
            let flier_spend = (flier_count as u32)
                ...
                .min(u16::MAX as u32) as u16;
            let total_spend = entry.intentional_spend.saturating_add(flier_spend);
            let with_candidate = add_vectors(entry.completed_x, candidate_x);
            let final_x = add_replacements(with_candidate, flier_count);
            buy_choices.push(EvaluatedChoice {
                evaluation: self.evaluate_dynamic_state(final_x, &frontier.opponent_sums),
                total_future_spend: total_spend,
                strategy_weights: entry.strategy_weights,
        ...
        for price in config.minimum_bid..=max_bid {
            ...
            if choice.evaluation.matchup_win_probability + H_INDIFFERENCE_EPSILON
                >= pass_best.evaluation.matchup_win_probability
            {
                fair_price = price;
                fair_choice = Some(choice);
            }
        ...
        Some(FullPricingBidAdvice {
            fair_price,
            expected_edge: fair_price as i16 - competition_price as i16,
            pass_projected_matchup_win_probability: pass_best.evaluation.matchup_win_probability,
            prep_market_projected_matchup_win_probability: prep_choice
                .evaluation
                .matchup_win_probability,
            competition_projected_matchup_win_probability: competition_choice
            ...
            frontier_market_seconds: frontier.market_seconds,
            frontier_beam_seconds: frontier.beam_seconds,
        })
    }

    pub(crate) fn roster_plan_validation_funnel_sweep(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
        ...
        // Search rankings are shared by every K and by the full-market
        // reference.  This is exactly the expensive matrix we do NOT want to
        // rebuild six times merely to test a different focus width.
        let search_strategies = search_strategy_weights
            .iter()
            .copied()
            .map(|weights| RolloutStrategy { weights })
        ...
        // Full-focus reference.  We intentionally call the focused-market
        // implementation with all candidates rather than the 64-strategy
        // competition market so K and reference use the SAME 16-strategy
        // future-pricing core. Only focus width changes.
        let reference_started = std::time::Instant::now();
        let reference_economy = self.focused_rational_opponent_market_economy(
            opponent_rosters,
            opponent_budgets_remaining,
            candidates,
            candidates,
        ...
        let reference_rollouts = self.top_intentional_auction_rollouts(
            own_roster.len(),
            own_sum,
            own_open_slots,
            own_budget_remaining,
        ...
        let reference_plan = self.validation_plan_from_rollouts(current, reference_rollouts)?;
        let reference_seconds = reference_started.elapsed().as_secs_f64();

        let mut ks = k_values.to_vec();
        ks.sort_unstable();
        ks.dedup();
        let mut points = Vec::<StrategyFunnelPoint>::with_capacity(ks.len());

        for k in ks {
            if k == 0 {
                continue;
            }
            let coarse_count = k.min(coarse.len());
            let mut focus_seen = HashSet::<PlayerId>::new();
            let mut focus_players = Vec::<PlayerId>::new();
            for rollout in coarse.iter().take(coarse_count) {
            ...
            // Critical v42 change versus v30: after focused repricing, rerun
            // ALL active j vectors, not merely the original coarse finalists.
            let rollouts = self.top_intentional_auction_rollouts(
                own_roster.len(),
                own_sum,
                own_open_slots,
                own_budget_remaining,
            ...
            let plan = self.validation_plan_from_rollouts(current, rollouts)?;
            points.push(StrategyFunnelPoint {
                k,
                coarse_count,
                focus_player_count: focus_players.len(),
                projected_h: plan.projected_matchup_win_probability,
                seconds: point_started.elapsed().as_secs_f64(),
            });
        }

        Some(StrategyFunnelSweep {
            reference_h: reference_plan.projected_matchup_win_probability,
            reference_seconds,
            coarse_seconds,
            points,
        })
    ...
    /// Search supplied strategies under one already-built market economy.
    /// The first 10 total roster spots receive the expensive strategy search;
    /// any remaining slots are still completed with real affordable players.
    /// This keeps the old runtime bound without pretending the 13-man roster is
    /// strategically only ten players deep.
    fn top_intentional_auction_rollouts(
        ...
        //
        // v77: only the first INTENTIONAL_ROSTER_SLOTS receive the expensive
        // strategic search treatment, but ALL remaining roster slots are now
        // occupied by actual affordable players before we fall back to a
        // replacement placeholder.  This is important economically: a saved
        // dollar is useful only while an open roster slot remains, and every
        // candidate must displace a real alternative occupant of one of the
        // team's finite 13 slots.
        let evaluate = |strategy: &RolloutStrategy, ranked_players: &Vec<PlayerId>| {
            ...
            // The old model hard-coded every slot after the first ten as a $1
            // generic flier.  That made the optimizer effectively reason about
            // a ten-player strategic roster and could strand large amounts of
            // cash after cheap wins.  Keep the expensive branching limited to
            // the first ten, but greedily fill EVERY remaining slot with the
            // best still-affordable real player from the same j ranking.  Only
            // use the replacement placeholder when no real player can fit.
            while future_players.len() < future_slots {
                let slots_after_this_purchase =
                    future_slots.saturating_sub(future_players.len() + 1);
                let reserve_after =
                    (slots_after_this_purchase as u32).saturating_mul(config.minimum_bid as u32);
                let max_spend = (budget as u32)
                    .saturating_sub(reserve_after)
                ...
                if let Some((player_id, x, price)) = chosen {
                    completed_x = add_vectors(completed_x, x);
                    budget = budget.saturating_sub(price);
                    future_spend = future_spend.saturating_add(price);
                    future_players.push(player_id.clone());
                ...
                // No real affordable player remains in this strategy ranking.
                // Finish the unresolved slots at replacement level so the H
                // comparison still always represents a complete 13-man team.
                let unresolved = future_slots.saturating_sub(future_players.len());
                for _ in 0..unresolved {
                    ...
                    future_player_names
                        .push(format!("Replacement-level (${})", config.minimum_bid));
                }
                break;
            }
    ...
    /// Combination-aware roster search used by the interactive full-market
    /// Strategy planner.
    ///
    /// The ordinary rollout deliberately follows one greedy path per j because
    /// it is called in many latency-sensitive and nested pricing contexts.  In
    /// Strategy we can afford more work: at every intentional roster slot we
    /// keep a small beam of partial rosters and branch over several of the best
    /// still-affordable players.  Each branch is ranked by a greedy completion
    /// look-ahead scored with the real H objective.  This lets combinations
    /// such as two $25-$30 players survive alongside one $50-$60 star without
    /// attempting the combinatorial full roster search.
    fn top_intentional_auction_rollouts_beam(
        &self,
        roster_size_at_start: usize,
        starting_x: [f64; 9],
        future_slots: usize,
        ...
        if keep == 0 || cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
            return Vec::new();
        }
        ...
        struct BeamState {
            ...
            // Player subsets are represented canonically in j-ranking order.
            // This avoids exploring A+B and B+A as separate auction paths.
            next_rank_index: usize,
        }
        ...
        let evaluate_strategy = |strategy: &RolloutStrategy, ranked_players: &Vec<PlayerId>| {
            if cancel.is_some_and(|flag| flag.load(Ordering::Relaxed)) {
                return Vec::<AuctionRolloutResult>::new();
            }
            // Complete a partial roster greedily only as a look-ahead
            // heuristic. The beam itself is what supplies alternate
            // combinations; the final H score is always evaluated from the
            // resulting completed roster.
            let complete = |partial: &BeamState| {
                let mut state = partial.clone();
                let mut cursor = state.next_rank_index;
                let mut intentional_index = state.future_players.len();

                while intentional_index < intentional_remaining {
                    let slots_after_this_purchase =
                        future_slots.saturating_sub(intentional_index + 1);
                    let reserve_after = (slots_after_this_purchase as u32)
                        .saturating_mul(config.minimum_bid as u32);
                    ...
                    state.completed_x = add_vectors(state.completed_x, x);
                    state.budget = state.budget.saturating_sub(price);
                    state.future_spend = state.future_spend.saturating_add(price);
                    state.future_players.push(player_id.clone());
                    if let Some(score) = self.score_for(player_id) {
                        state
                            .future_player_names
                            .push(format!("{} (${price})", score.player_name));
                    }
                    state.next_rank_index = rank_index + 1;
                    intentional_index += 1;
                }

                // v77 slot-aware tail: expensive branching still stops at
                // INTENTIONAL_ROSTER_SLOTS, but the remaining roster spots are
                // not imaginary $1 placeholders. Greedily spend the ACTUAL
                // remaining budget on the best affordable real players in this
                // strategy ranking, reserving $1 for every still-open slot.
                // This lets cheap early wins translate into stronger final-slot
                // occupants instead of stranded cash.
                while state.future_players.len() < future_slots {
                    let slots_after_this_purchase =
                        future_slots.saturating_sub(state.future_players.len() + 1);
                    let reserve_after = (slots_after_this_purchase as u32)
                        .saturating_mul(config.minimum_bid as u32);
                    let max_spend = (state.budget as u32)
                        .saturating_sub(reserve_after)
                    ...
                    if let Some((rank_index, player_id, x, price)) = chosen {
                        state.completed_x = add_vectors(state.completed_x, x);
                        state.budget = state.budget.saturating_sub(price);
                        state.future_spend = state.future_spend.saturating_add(price);
                        state.future_players.push(player_id.clone());
                    ...
                    let unresolved = future_slots.saturating_sub(state.future_players.len());
                    for _ in 0..unresolved {
                        if state.budget < config.minimum_bid {
                            break;
                        }
                        state.completed_x =
                            add_vectors(state.completed_x, economy.snapshot.replacement_x_scores);
                        state.budget = state.budget.saturating_sub(config.minimum_bid);
                        state.future_spend = state.future_spend.saturating_add(config.minimum_bid);
                        state
                            .future_player_names
                            .push(format!("Replacement-level (${})", config.minimum_bid));
                    }
                ...
                let evaluation = self.evaluate_dynamic_state(state.completed_x, opponent_sums);
                AuctionRolloutResult {
                    strategy: strategy.clone(),
                    evaluation,
                    future_players: state.future_players,
                    future_player_names: state.future_player_names,
                    future_spend: state.future_spend,
                    budget_left: state.budget,
                }
            };

            let mut beam = vec![BeamState {
                completed_x: starting_x,
                budget: starting_budget,
                future_spend: 0,
                future_players: Vec::with_capacity(future_slots),
                future_player_names: Vec::with_capacity(future_slots),
                next_rank_index: 0,
            }];
            ...
            for intentional_index in 0..intentional_remaining {
                ...
                let slots_after_this_purchase = future_slots.saturating_sub(intentional_index + 1);
                let mut children = Vec::<ScoredBeamState>::with_capacity(
                    beam.len().saturating_mul(BRANCHES_PER_STATE),
                );
                ...
                for state in &beam {
                    ...
                    for rank_index in state.next_rank_index..ranked_players.len() {
                        ...
                        child.completed_x = add_vectors(child.completed_x, x);
                        child.budget = child.budget.saturating_sub(price);
                        child.future_spend = child.future_spend.saturating_add(price);
                        child.future_players.push(player_id.clone());
                        if let Some(score) = self.score_for(player_id) {
                            child
                                .future_player_names
                        ...
                        let preview_result = complete(&child);
                        children.push(ScoredBeamState {
                            state: child,
                            preview: preview_result.evaluation,
                            preview_spend: preview_result.future_spend,
                        });
                        branches += 1;
                    }
                }
                ...
                beam = children.into_iter().map(|child| child.state).collect();
            }
            ...
            let mut results = beam
                .iter()
                .map(complete)
                .collect::<Vec<AuctionRolloutResult>>();
            sort_and_truncate_auction_rollouts(&mut results, keep);
            results
        };
        ...
        if strategies.len() >= PARALLEL_ROLLOUT_THRESHOLD {
            let mut best = strategies
                .par_iter()
                .zip(strategy_rankings.par_iter())
                .map(|(strategy, ranked_players)| evaluate_strategy(strategy, ranked_players))
                .fold(
                    || Vec::<AuctionRolloutResult>::with_capacity(keep + 1),
                    |mut local_best, mut results| {
                        local_best.append(&mut results);
                        sort_and_truncate_auction_rollouts(&mut local_best, keep);
                        local_best
                    },
                )
        ...
        let mut best = Vec::<AuctionRolloutResult>::with_capacity(keep + 1);
        for (strategy, ranked_players) in strategies.iter().zip(strategy_rankings.iter()) {
            let mut results = evaluate_strategy(strategy, ranked_players);
            best.append(&mut results);
            sort_and_truncate_auction_rollouts(&mut best, keep);
        }
        best
    }
        ...
        let mut scores = candidates
            ...
            .filter_map(|candidate| {
                ...
                let auction_score = AuctionDurantScore {
                    player_id: candidate.clone(),
                    player_name: static_score.player_name.clone(),
                    prep_market_price: market_price,
                    evaluated_prep_market_price: analysis_price,
                    ...
                    expected_edge,
                    pass_projected_matchup_win_probability: pass_h,
                    prep_market_projected_matchup_win_probability: market_rollout
                        .evaluation
                        .matchup_win_probability,
    ...
    /// now gives all 13 roster slots real occupants. Only the first 10 receive
    /// expensive branching/repricing; the tail is a cheap greedy completion.
    /// The expensive rational-opponent market and sequential rational repricing
    /// are reserved for the single-player Enter calculation.
    ...
    /// future-market refinement. The first 10 roster spots receive expensive
    /// strategic repricing; the remaining slots are still filled with actual
    /// affordable players so all 13 roster slots carry opportunity cost.
    ///
    ...
    pub fn auction_candidate_analysis_with_strategy_weights(
        ...
        let evaluated_prep_market_price = prep_market_price.clamp(config.minimum_bid, max_bid);
        let prep_market_h = evaluate_buy(evaluated_prep_market_price)
            .map(|result| result.evaluation.matchup_win_probability)
            .unwrap_or(pass_h);
        ...
        Ok(Some(AuctionDurantScore {
            ...
            expected_edge,
            pass_projected_matchup_win_probability: pass_h,
            prep_market_projected_matchup_win_probability: prep_market_h,
            market_price_projected_matchup_win_probability: market_rollout
                .evaluation
                .matchup_win_probability,
            fair_price_projected_matchup_win_probability: if fair_price == 0 {
    ...
    /// Offline synthetic-auction helper used by the independent market census.
    ///
    /// This deliberately avoids a recursive market-price fixed point. Every
    /// manager solves the same H-indifference max-bid problem as live BirdBoard,
    /// while future roster completion uses BirdBoard's transparent legacy
    /// replacement/G-dollar market for the CURRENT room state as a neutral
    /// opportunity-cost prior. The observed synthetic sale price still comes
    /// from competing rational max bids.
    pub(crate) fn synthetic_room_reservation_bids_with_strategy_weights(
        &self,
        rosters: &[Vec<PlayerId>],
        budgets_remaining: &[u16],
        candidate: &PlayerId,
        candidates: &[PlayerId],
        team_strategy_weights: &[[f64; 9]],
        config: AuctionConfig,
    ) -> Vec<u16> {
        let team_count = rosters
            .len()
            .min(budgets_remaining.len())
            .min(team_strategy_weights.len());
        if team_count == 0 || candidates.is_empty() {
            return Vec::new();
        }
        let Some(candidate_score) = self.score_for(candidate) else {
            return vec![0; team_count];
        };

        let mut remaining_slots = 0usize;
        let mut remaining_dollars = 0u32;
        let mut max_legal_market_bid = 0u16;
        for team in 0..team_count {
            let open = self.team_size.saturating_sub(rosters[team].len());
            remaining_slots = remaining_slots.saturating_add(open);
            remaining_dollars = remaining_dollars.saturating_add(budgets_remaining[team] as u32);
            max_legal_market_bid = max_legal_market_bid.max(max_legal_bid(
                budgets_remaining[team],
                open,
                config.minimum_bid,
            ));
        }
        let market_totals = AuctionMarketTotals {
            remaining_slots,
            remaining_dollars,
            max_legal_market_bid,
        };
        ...
        // A room often contains repeated manager profiles. Rank the remaining
        // player pool once per distinct profile and reuse it for every manager
        // with that profile.
        let mut profile_cache = Vec::<([f64; 9], RolloutStrategy, Vec<PlayerId>)>::new();
        for weights in team_strategy_weights.iter().take(team_count) {
            if profile_cache.iter().any(|(known, _, _)| known == weights) {
                continue;
            }
            let strategy = RolloutStrategy { weights: *weights };
            let ranking = self
                .rank_available_by_strategy(candidates, std::slice::from_ref(&strategy))
                .into_iter()
                .next()
                .unwrap_or_default();
            profile_cache.push((*weights, strategy, ranking));
        }
        ...
        (0..team_count)
            .map(|team| {
                ...
                let weights = team_strategy_weights[team];
                let Some((_, strategy, ranking)) =
                    profile_cache.iter().find(|(known, _, _)| *known == weights)
                else {
                    return 0;
                };

                self.rational_bid_for_team(
                    &rosters[team],
                    budgets_remaining[team],
                    candidate,
                    candidate_score,
    ...
    /// Fast single-team H-indifference reservation bid for simulation/live-policy
    /// validation.
    ///
    /// This is deliberately separate from MARKET.  `self` supplies the expected
    /// future-price environment (legacy, ESPN-shaped, or BirdBoard synthetic),
    /// while the returned number is the team's own willingness to pay:
    ///
    ///     max p such that H(BUY candidate @ p) >= H(PASS candidate)
    ///
    /// PASS and BUY both complete the roster under the same price prior and
    /// against the actual opponent rosters.  No opponent reservation bid is
    /// solved here; the surrounding auction simulator provides competition and
    /// the eventual second-price clearing price.
    pub(crate) fn current_state_reservation_bid_with_strategy_weights(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
        opponent_rosters: &[Vec<PlayerId>],
    ...
    /// Offline preseason reservation bid used by the independent market census.
    ///
    /// This is the SAME H-indifference idea as live Max Bid, but evaluated for
    /// a synthetic manager in a fresh 13-team room.  Unlike the old v72 market
    /// generator, this does not split a team's $187 discretionary dollars across
    /// 13 simultaneous targets.  It asks the economically correct question for
    /// each candidate independently:
    ///
    ///     max p such that H(BUY candidate @ p) >= H(PASS candidate)
    ///
    /// `prior_prices` is only the neutral future-price environment used while
    /// completing BUY/PASS rosters.  The market builder iterates this prior to a
    /// fixed point; epoch 0 deliberately starts from the legacy statistical
    /// market only as an initialization, never from ESPN.
    pub(crate) fn preseason_reservation_bid_with_strategy_weights(
        &self,
        candidate: &PlayerId,
        candidates: &[PlayerId],
        strategy_weights: &[[f64; 9]],
        prior_prices: Option<&HashMap<PlayerId, f64>>,
        config: AuctionConfig,
    ) -> u16 {
        if strategy_weights.is_empty() || candidates.is_empty() {
            return 0;
        }
        let Some(candidate_score) = self.score_for(candidate) else {
            return 0;
        ...
        let open_slots = self.team_size;
        let max_bid = max_legal_bid(config.starting_budget, open_slots, config.minimum_bid);
        if max_bid < config.minimum_bid {
            return 0;
        }

        let room_slots = self.league_teams().saturating_mul(self.team_size);
        let room_dollars =
            (self.league_teams() as u32).saturating_mul(config.starting_budget as u32);
        let market_totals = AuctionMarketTotals {
            remaining_slots: room_slots,
            remaining_dollars: room_dollars,
            max_legal_market_bid: max_bid,
        };

        // IMPORTANT: never let an already-saved equilibrium file leak into the
        // new market build.  Epoch 0 uses the legacy statistical curve only as
        // an initialization.  Later epochs use the previous rational-bid curve.
        let economy = if let Some(prior) = prior_prices.filter(|prior| !prior.is_empty()) {
            self.estimate_market_economy_from_equilibrium_prices(
                candidates,
                None,
                market_totals,
                prior,
            ...
            self.estimate_legacy_market_economy_from_totals(candidates, None, market_totals, config)
        };
        ...
        let strategies = strategy_weights
            ...
            .collect::<Vec<_>>();
        let strategy_rankings = self.rank_available_by_strategy(candidates, &strategies);
        let team_sum = self.aggregate_x_scores(&[]);
        let generic_field = [([0.0; 9], 0usize)];
    ...
    /// bids. That keeps the broad Draft screen responsive while still completing
    /// all 13 finite roster slots with real affordable players when possible.
    fn refine_rollout_with_static_future_market(
        ...
        // v77: finish the entire finite roster with real players whenever
        // possible.  We keep branching/search expensive only for the first ten
        // strategic slots; the tail is a cheap greedy completion.  Crucially,
        // saved budget can now upgrade those last slots instead of vanishing
        // into three hard-coded $1 replacement placeholders.
        let mut completed_future_slots = future_players.len();
        while completed_future_slots < future_slots {
            if budget < config.minimum_bid {
                break;
            }
            let slots_after_this_purchase = future_slots.saturating_sub(completed_future_slots + 1);
            let reserve_after =
                (slots_after_this_purchase as u32).saturating_mul(config.minimum_bid as u32);
            let max_spend = (budget as u32)
                .saturating_sub(reserve_after)
            ...
            if let Some((player_id, x, price)) = ranked.into_iter().find_map(|(player_id, x, _)| {
                let price = economy
                    .prices
                    .get(&player_id)
                    .copied()
                ...
                market_totals.remaining_dollars =
                    market_totals.remaining_dollars.saturating_sub(price as u32);
                completed_future_slots += 1;
                continue;
            }

            // If no actual player can fit the legal reserve, no later slot can
            // become easier to fund.  Fill the rest at replacement level.
            let unresolved = future_slots.saturating_sub(completed_future_slots);
            for _ in 0..unresolved {
                if budget < config.minimum_bid {
                    break;
                }
                ...
                budget = budget.saturating_sub(config.minimum_bid);
                future_spend = future_spend.saturating_add(config.minimum_bid);
                future_player_names.push(format!("Replacement-level (${})", config.minimum_bid));
                market_totals.remaining_slots = market_totals.remaining_slots.saturating_sub(1);
    ...
    ///
    /// Only the first 10 total roster spots receive expensive sequential
    /// opponent repricing. Remaining slots are completed cheaply with actual
    /// affordable players so they still exert roster-slot opportunity cost.
    fn refine_rollout_with_dynamic_future_market(
        ...
        for intentional_index in 0..intentional_remaining {
            ...
            let Some((player_id, x, price)) = chosen else {
                // If no expensive-core target is affordable, stop the
                // repriced phase. The cheap slot-aware tail below still tries
                // to fill every remaining roster spot with a real player.
                break;
            };
        ...
        // v77 slot-aware tail.  Full opponent repricing remains reserved for
        // the first ten strategic purchases, but the remaining slots are still
        // REAL scarce roster slots.  Fill them greedily from the remaining
        // player pool at the current non-recursive market, using any budget
        // saved by earlier bargains to improve the final roster.
        let mut completed_future_slots = future_players.len();
        while completed_future_slots < future_slots {
            if budget < config.minimum_bid {
                break;
    ...
    fn estimate_market_economy_from_totals(
        ...
        if let Some(prices) = self
            .absolute_market_prices
            .as_ref()
            .filter(|prices| !prices.is_empty())
        {
            return self.estimate_market_economy_from_absolute_prices(
                candidates,
                excluded_player,
                totals,
                prices,
        ...
        if let Some(prices) = self
            ...
            .filter(|prices| !prices.is_empty())
        {
            return self.estimate_market_economy_from_equilibrium_prices(
                candidates,
                excluded_player,
                totals,
                prices,
        ...
        self.estimate_legacy_market_economy_from_totals(candidates, excluded_player, totals, config)
    }

    fn estimate_legacy_market_economy_from_totals(
        &self,
        candidates: &[PlayerId],
        excluded_player: Option<&PlayerId>,
        totals: AuctionMarketTotals,
    ...
    /// Build a live market from an ABSOLUTE external dollar table.  The table
    /// is not forced to consume all room dollars.  Instead, current room
    /// inflation is measured relative to the same table's INITIAL cash/value
    /// ratio, so the opening state always has multiplier 1.0.
    fn estimate_market_economy_from_absolute_prices(
        &self,
        candidates: &[PlayerId],
        excluded_player: Option<&PlayerId>,
        totals: AuctionMarketTotals,
        absolute_prices: &HashMap<PlayerId, f64>,
        config: AuctionConfig,
    ) -> MarketEconomy {
        let mut available = candidates
            .iter()
            ...
            .filter_map(|player_id| {
                let score = self.score_for(player_id)?;
                let price = absolute_prices
                    .get(player_id)
                    .copied()
                    .unwrap_or(config.minimum_bid as f64);
                Some((player_id.clone(), score.total, price))
            })
            .collect::<Vec<_>>();

        if available.is_empty() {
            return self.estimate_legacy_market_economy_from_totals(
                candidates,
                excluded_player,
                totals,
                config,
        ...
        let draftable_count = totals.remaining_slots.min(available.len());
        let mut by_g = available.clone();
        by_g.sort_by(|a, b| b.1.total_cmp(&a.1));
        let replacement_g_score = if by_g.len() > draftable_count {
            by_g[draftable_count].1
        } else {
            by_g.last().map(|(_, g, _)| *g).unwrap_or(0.0)
        };

        let cutoff = draftable_count.min(by_g.len());
        let replacement_band = self.league_teams().max(1);
        let band_start = cutoff.saturating_sub(replacement_band / 2);
        let band_end = (band_start + replacement_band).min(by_g.len());
        let mut replacement_x_scores = [0.0; 9];
        let mut replacement_count = 0usize;
        for (player_id, _, _) in &by_g[band_start..band_end] {
            if let Some(x) = self.x_score_for(player_id) {
                replacement_x_scores = add_vectors(replacement_x_scores, x);
                replacement_count += 1;
            }
        ...
        let total_g_above_replacement = by_g
            .iter()
            .take(draftable_count)
            .map(|(_, g, _)| (g - replacement_g_score).max(0.0))
            .sum::<f64>();
        let g_above_replacement_per_dollar = if discretionary_dollars == 0 {
            0.0
        } else {
        ...
        // Price-ranked mass represents what the external market expects the
        // remaining rosterable players to absorb.  Compare its live cash/value
        // ratio with the INITIAL ratio stored by the setter.  This removes the
        // old validation bug where a truncated historical player pool caused
        // immediate ~1.5x inflation of every ESPN price.
        ...
        let minimum = config.minimum_bid as f64;
        let live_value_mass = available
            .iter()
            .take(draftable_count)
            .map(|(_, _, price)| (*price - minimum).max(0.0))
            .sum::<f64>();
        let live_ratio = if live_value_mass <= f64::EPSILON {
            None
        } else {
            Some(discretionary_dollars as f64 / live_value_mass)
        };
        let reference_ratio = self.absolute_market_reference_ratio.unwrap_or(1.0);
        let inflation = match live_ratio {
            Some(live_ratio) if reference_ratio > f64::EPSILON => {
                (live_ratio / reference_ratio).clamp(0.65, 1.50)
            }
            _ => 1.0,
        };
        ...
        let mut exact_prices = HashMap::new();
        let mut prices = HashMap::new();
        for (rank, (player_id, _, base_price)) in available.into_iter().enumerate() {
            let exact = if rank < draftable_count {
                (minimum + (base_price - minimum).max(0.0) * inflation).clamp(minimum, market_cap)
            } else {
                minimum
            };
            exact_prices.insert(player_id.clone(), exact);
    ...
    /// Reprice the current room from a saved, independently simulated
    /// equilibrium curve.
    ///
    /// The offline equilibrium determines only the SHAPE of demand. Live
    /// remaining dollars are still conserved exactly (before legal-bid caps),
    /// so inflation/deflation follows the actual room instead of a frozen
    /// preseason dollar table. Players absent from the saved file fall back to
    /// the legacy G-above-replacement weight rather than becoming free.
    fn estimate_market_economy_from_equilibrium_prices(
        &self,
        candidates: &[PlayerId],
        excluded_player: Option<&PlayerId>,
        totals: AuctionMarketTotals,
        ...
        let available = candidates
            ...
            .filter_map(|player_id| {
                ...
                Some((
                    player_id.clone(),
                    score.total,
                    equilibrium_prices.get(player_id).copied(),
                ))
            })
            .collect::<Vec<_>>();
        ...
        let minimum = config.minimum_bid as f64;
        let fallback_dollars_per_g = if total_g_above_replacement <= f64::EPSILON {
            0.0
        } else {
            discretionary_dollars as f64 / total_g_above_replacement
        };

        let mut weighted = available
            .into_iter()
            .map(|(player_id, g, saved_price)| {
                let fallback = (g - replacement_g_score).max(0.0) * fallback_dollars_per_g;
                let weight = saved_price
                    .map(|price| (price - minimum).max(0.0))
                    .unwrap_or(fallback);
                (player_id, g, weight)
            })
            .collect::<Vec<_>>();
        ...
        let mut exact_prices = HashMap::new();
        let mut prices = HashMap::new();
        for (rank, (player_id, _, weight)) in weighted.into_iter().enumerate() {
            let exact = if rank < draftable_count {
                (minimum + discretionary_dollars as f64 * weight / total_weight)
                    .clamp(minimum, market_cap)
            } else {
                minimum
            };
    ...
    fn evaluate_dynamic_state(
        ...
        let mut evaluation = self.evaluate_dynamic_state_raw(own_x, opponent_sums);
        // Apply calibration only after averaging raw H over opponents. Because
        // the transform is strictly monotonic, this preserves the optimizer's
        // ordering while making the reported probability empirically honest.
        evaluation.matchup_win_probability =
            self.h_calibration.apply(evaluation.matchup_win_probability);
        evaluation
    }

    fn evaluate_dynamic_state_raw(
        &self,
        own_x: [f64; 9],
        opponent_sums: &[([f64; 9], usize)],
    ) -> DynamicStateEvaluation {
...
fn load_h_calibration(cache_dir: &std::path::Path) -> HCalibration {
    let path = cache_dir.join("durant").join(H_CALIBRATION_FILENAME);
    let Ok(text) = fs::read_to_string(&path) else {
        return HCalibration::default();
    };
    let Ok(calibration) = serde_json::from_str::<HCalibration>(&text) else {
        eprintln!(
            "warning: could not parse H calibration {}; using raw H",
            path.display()
        );
        return HCalibration::default();
    };
    if calibration.schema_version != 1 || !calibration.slope.is_finite() || calibration.slope <= 0.0
    {
        eprintln!(
            "warning: invalid H calibration {}; using raw H",
            path.display()
        );
        return HCalibration::default();
    }
...
fn fingerprint_model(
    ...
    h_calibration.slope.to_bits().hash(&mut hasher);
...
const AUCTION_SEARCH_VERSION: u32 = 4;
const AUCTION_J_CENSUS_VERSION: u32 = 3;
...
/// In a 13-man roster the first 10 spots receive the expensive strategic
/// branching/repricing treatment. v77 still fills the remaining three slots
/// with actual affordable players; this constant is now a COMPUTE boundary,
/// not a claim that the final slots are generic $1 placeholders.
