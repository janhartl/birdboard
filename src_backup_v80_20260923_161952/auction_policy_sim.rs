...
fn independent_equilibrium_market_vs_espn() -> Result<()> {
    ...
    println!(
        "ESPN opponents use current ESPN averages + RELATIVE live room inflation anchored to 1.0 at the opening state + mild stable roster preference."
    );
    println!(
    ...
    println!(
        "Auction semantics v80: MARKET predicts future cost; ESPN dollar priors remain absolute (no full-budget rescaling); Max Bid is H-indifference, target membership never caps bidding, off-plan bargains may be bought, and every nomination carries the nominator's mandatory $1 opening bid."
    );
    println!(
