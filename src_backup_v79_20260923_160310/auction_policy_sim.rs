...
fn independent_equilibrium_market_vs_espn() -> Result<()> {
    ...
    println!(
        "Auction semantics v79: MARKET predicts future cost; Max Bid is H-indifference, target membership never caps bidding, off-plan bargains may be bought, and every nomination carries the nominator's mandatory $1 opening bid."
    );
    println!(
...
fn simulate_competitive_espn_room(
    ...
    // v78: MARKET is a price prior, never a bid ceiling.  Give each matched
    ...
    if mode == CompetitiveBirdBoardBidMode::EspnClearingMarket {
        birdboard_model.set_equilibrium_prices(Some(espn_prices.clone()));
    }
    let opponent_styles = (0..TEAM_COUNT)
        .map(|team| {
            let index =
