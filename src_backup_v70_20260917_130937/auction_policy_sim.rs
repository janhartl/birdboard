...
impl CompetitiveWeeklyAggregate {
    fn push_room_budget(&mut self, state: &SimState) {
        ...
        self.field_budget_left_sum +=
            state.budgets.iter().copied().sum::<u16>() as f64 / TEAM_COUNT as f64;
        self.max_team_budget_left = self
            .max_team_budget_left
            .max(state.budgets.iter().copied().max().unwrap_or(0));
...
fn espn_competitive_auction_vs_birdboard_strategy() -> Result<()> {
    ...
    println!(
        "Two BirdBoard variants are run on matched seeds: TEAM strategy at the ESPN clearing market, and TEAM strategy with BirdBoard's native market ceiling.\n"
    );
    ...
    for room in 0..rooms {
        ...
        for mode in CompetitiveBirdBoardBidMode::ALL {
            ...
            let avg_left = total_left as f64 / TEAM_COUNT as f64;
            let max_left = state.budgets.iter().copied().max().unwrap_or(0);
            ...
            println!(
                "  {:<22} weekly #1 {:>1}/5 | avg rank {:>4.1} | BB left ${:>3} | field avg left ${:>4.1} | max left ${:>3} | spend ${}",
                mode.label(),
                room_wins,
    ...
    println!(
        "TEAM @ native market is the tougher END-TO-END check: BirdBoard must both choose the right players and value them well enough to win real bids."
    );
    println!(
...
fn birdboard_competitive_max_bid(
    ...
    let base = match mode {
        CompetitiveBirdBoardBidMode::EspnClearingMarket => {
            competitive_espn_expected_price(espn_prices, player_id, inflation)
        }
        CompetitiveBirdBoardBidMode::NativeBirdBoardMarket => {
            native_prices.get(player_id).copied().unwrap_or(MIN_BID) as f64
        }
    };

    // The planner's first few names are genuine acquisition targets rather
    // than interchangeable depth. A small, bounded willingness premium lets
    // BirdBoard actually contest those targets without turning the simulation
    // into "always pay whatever it takes".
