...
pub fn run_frontier(app: &App) -> Result<()> {
    ...
    let pass_stage = RuntimeStrategyStage::for_roster_size(own_roster.len());
    let pass_weights = stage_weights(
        app,
        &own_roster,
        own_budget,
    ...
    let evaluated_competition = fast
        .as_ref()
        .map(|score| score.evaluated_market_price)
        .unwrap_or(competition.min(own_budget));

    let mut buy_roster = own_roster.clone();
    buy_roster.push(target_id.clone());
    let buy_budget_reference = own_budget.saturating_sub(evaluated_competition);
    let buy_stage = RuntimeStrategyStage::for_roster_size(buy_roster.len());
    let buy_weights = stage_weights(
        app,
        &buy_roster,
    ...
    println!("========================================================================");
    println!("CANDIDATE-SPECIFIC STRATEGY BUDGET FRONTIER");
    println!("========================================================================");
    println!("target        : {target_display}");
    println!("own roster    : {} players", own_roster.len());
    println!("own budget    : ${own_budget}");
    ...
    let frontier = app
        .durant
        .build_candidate_pricing_frontier_with_strategy_weights(
            &own_roster,
            own_budget,
            &opponent_rosters,
            &opponent_budgets,
        ...
        .context("candidate-specific Strategy frontier found no valid completion states")?;
    let advice = app
        .durant
        .candidate_pricing_bid_from_frontier(
            &frontier,
            &target_id,
            prep_market,
            competition,
        ...
        .context("candidate-specific frontier could not price target")?;
    ...
    println!("  pass+buy beam: {:.2}s", advice.frontier_beam_seconds);
    println!();
    println!("STRATEGY max   : ${}", advice.fair_price);
    ...
    println!();
    println!("This is the candidate-specific budget-frontier path used by the live nomination UI.");
    println!(
        "Compare its Max Bid to the v52 repeated full-beam reference for the same player/state."
    );
