...
fn render_player_detail(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if matches!(app.session_phase, SessionPhase::LiveDraft) {
        ...
        if let Some(score) = app.live_advantage_for(&player.id) {
            ...
            if score.has_projected_finish {
                ...
                lines.push(Line::from(Span::styled(
                    "FINAL/build projected for the top 25 NOW options; Enter runs the nomination model.",
                    Style::default().add_modifier(Modifier::DIM),
                )));
            }
        }
        ...
        if app.full_pricing_frontier_loading {
            lines.push(Line::from(Span::styled(
                format!(
                    "PARETO bid cache {} {:.1}s",
                    app.full_pricing_frontier_spinner(),
                    app.full_pricing_frontier_elapsed_seconds()
                ),
                Style::default().add_modifier(Modifier::DIM),
            )));
        } else if app.full_pricing_frontier_ready() {
            let elapsed = app
                .full_pricing_frontier_ms
                .map(|ms| format!(" · {:.1}s", ms as f64 / 1000.0))
                .unwrap_or_default();
            lines.push(Line::from(Span::styled(
                format!("PARETO bid cache READY{elapsed}"),
                Style::default().add_modifier(Modifier::DIM),
            )));
        } else if let Some(error) = &app.full_pricing_frontier_error {
            lines.push(Line::from(Span::styled(
                format!("PARETO bid cache: {error}"),
                Style::default().add_modifier(Modifier::DIM),
            )));
        }
...
fn render_nomination_panel(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(advice) = &app.nomination_advice {
        ...
        lines.push(Line::from(""));
        lines.push(heading("PRICING AUDIT · FAST"));
        lines.push(kv("Competition", &format!("${}", advice.market_price)));
        lines.push(kv("FAST max bid", &format!("${}", advice.fair_price)));
        ...
        if advice.fair_price == 0 {
            lines.push(kv("H @ FAST max", "—  no positive bid"));
        } else {
            lines.push(kv(
                &format!("H @ max ${}", advice.fair_price),
                &format_h_vs_pass(max_h),
            ));
        }
        lines.push(kv("Decision", decision));
        lines.push(Line::from(""));
        lines.push(kv("FAST build", &advice.build_name));
        ...
        lines.push(Line::from(""));
        lines.push(heading("PRICING AUDIT · PARETO FRONTIER"));
        if let Some(full) = &app.nomination_full_advice {
            ...
            lines.push(kv("PARETO max bid", &format!("${}", full.fair_price)));
            ...
            if full.fair_price == 0 {
                lines.push(kv("H @ PARETO max", "—  no positive bid"));
            } else {
                lines.push(kv(
            ...
            lines.push(kv("PARETO build", &full.build_name));
            lines.push(Line::from(Span::styled(
                format!(
                    "shared Pareto cache · market {:.1}s · beam {:.1}s",
                    full.frontier_market_seconds, full.frontier_beam_seconds
                ),
                Style::default().add_modifier(Modifier::DIM),
            )));
            ...
            lines.push(Line::from(Span::styled(
                format!(
                    "{} building shared Pareto cache... {:.1}s",
                    app.full_pricing_frontier_spinner(),
                    app.full_pricing_frontier_elapsed_seconds()
                ),
                Style::default().add_modifier(Modifier::DIM),
            ...
            lines.push(Line::from(Span::styled(
                "FAST competition remains available while the Pareto frontier computes.",
                Style::default().add_modifier(Modifier::DIM),
            )));
        } else if let Some(error) = &app.full_pricing_frontier_error {
            lines.push(Line::from(Span::styled(
            ...
            lines.push(Line::from(Span::styled(
                "Pareto pricing cache not ready for this draft state.",
                Style::default().add_modifier(Modifier::DIM),
            )));
        }
