...
use crate::durant::{
    ...
    FullPricingBidAdvice, FullPricingFrontier, MarketAdvantageScore, MarketBoard, MarketValue,
};
...
const LIVE_PROJECTED_SHORTLIST_LIMIT: usize = 25;
...
struct FullPricingFrontierJobResult {
    generation: u64,
    frontier: Option<FullPricingFrontier>,
    elapsed_ms: u128,
    error: Option<String>,
}
...
pub struct App {
    ...
    /// Shared fixed-market pricing cache. Built once per draft state on a
    /// worker thread, then every nomination can query frontier Max Bid cheaply.
    full_pricing_frontier: Option<FullPricingFrontier>,
...
impl App {
        ...
        match key.code {
            ...
            KeyCode::Char('s')
                if matches!(&self.draft_mode, DraftMode::BrowsingPlayers)
                    && matches!(&self.session_phase, SessionPhase::LiveDraft) =>
            {
                // Strategy is candidate-conditioned: open immediately, then
                // solve the full opponent-aware future roster for THIS selected
                // player on a worker thread. Re-entering Strategy from another
                // player must never reuse the previous player's plan.
                self.screen = Screen::Strategy;
                self.invalidate_strategy_full_plan();
                self.start_full_strategy_plan_async();
            }
        ...
        match phase {
            ...
            SessionPhase::LiveDraft => {
                if self.live_analysis_dirty || self.live_advantage_board.is_empty() {
                    ...
                    self.start_full_pricing_frontier_async();
                }
            }
        }
    }
    ...
    pub fn refresh_market_board(&mut self) {
        ...
        // The Draft screen gets a cheap Live refresh immediately; the much
        // heavier whole-roster plan is recomputed lazily only when the user
        // actually opens Rosters/Strategy.
        ...
        if matches!(self.session_phase, SessionPhase::LiveDraft) {
            ...
            // Start the shared Max Bid frontier immediately after every
            // meaningful draft-state change. It runs entirely off the UI thread.
            self.start_full_pricing_frontier_async();
        }
    ...
    fn start_full_pricing_frontier_async(&mut self) {
        ...
        let search_weights = self.active_strategy_weights.clone();
        ...
        thread::spawn(move || {
            ...
            let frontier = model.build_full_pricing_frontier_with_strategy_weights(
                &own_roster,
                own_budget,
                &opponent_rosters,
                &opponent_budgets,
            ...
            let error = frontier
                .is_none()
                .then(|| "Pareto pricing frontier found no valid completion states".to_string());
    ...
    fn refresh_full_nomination_from_frontier(&mut self) {
        ...
        let full = self.durant.full_pricing_bid_from_frontier(
            frontier,
            &player_id,
    ...
    fn invalidate_strategy_full_plan(&mut self) {
        // Dropping the receiver makes any stale worker result harmless. The
        // worker itself may finish, but its sender will simply observe a closed
        // channel. Generation is also bumped as an extra stale-result guard.
            ...
            match receiver.try_recv() {
                Ok(result) => {
                    ...
                    if result.generation == self.full_pricing_frontier_generation {
                        ...
                        self.full_pricing_frontier_started = None;
                        if has_frontier && matches!(self.draft_mode, DraftMode::RecordingDraft) {
                ...
                Err(TryRecvError::Disconnected) => {
                    ...
                    self.full_pricing_frontier_loading = false;
                    self.full_pricing_frontier_error =
                        Some("Pareto pricing frontier worker stopped unexpectedly".to_string());
                    self.full_pricing_frontier_started = None;
                }
            }
        }
