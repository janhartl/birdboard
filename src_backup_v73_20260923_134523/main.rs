mod app;
#[cfg(test)]
mod auction_policy_sim;
mod data;
mod debug;
mod draft;
mod durant;
mod equilibrium_market;
mod event;
mod h_calibration;
mod h_validation;
mod player;
...
fn main() -> Result<()> {
    ...
    match std::env::args().nth(1).as_deref() {
        ...
        Some("--frontier-price-check") => {
            pricing_validation::run_frontier(&app)?;
            return Ok(());
        }
        Some("--pricing-replay-info") => {
            pricing_validation::print_replay_plan();
            return Ok(());
        }
        Some("--pricing-replay-validate") => {
            pricing_validation::run_replay_validation(&app)?;
            return Ok(());
        }
