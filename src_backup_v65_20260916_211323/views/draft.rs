...
fn render_right_detail_panel(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if matches!(app.session_phase, SessionPhase::LiveDraft) && app.strategy_queue_len() > 0 {
        ...
        let team_plan_preview_rows = app
            ...
            .take(4)
            .filter_map(|entry| {
                if !matches!(entry.kind, crate::app::StrategyQueueKind::TeamPlan) {
                    return None;
                }
                entry
                    .plan
                    .as_ref()
                    .map(|plan| plan.projected_future_player_names.iter().take(4).count())
            })
            .sum::<usize>();
        let queue_height =
    ...
    for entry in app.strategy_queue.iter().take(4) {
        ...
        match entry.kind {
            crate::app::StrategyQueueKind::TeamPlan => {
                lines.push(Line::from(format!("{status} {:<23}{timing}", "TEAM PLAN")));
                if let Some(plan) = entry.plan.as_ref() {
                    for (index, player_name) in plan
