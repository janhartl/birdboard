...
pub fn draw(frame: &mut Frame, app: &App) {
    ...
    let footer = match app.session_phase {
        ...
        SessionPhase::LiveDraft => "[j/k] Eval queue   [b] Back to board   [r] Rosters   [h] Home",
    };
...
fn render_candidate_snapshot(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mut lines = vec![
        ...
        Line::from(format!(
            "Eval         {}",
            app.strategy_queue_entry_for(&player.id)
...
fn render_plan_summary(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if app.strategy_view_loading() {
        lines.push(Line::from(""));
        let queued = app
            .selected_player_ref()
            .and_then(|player| app.strategy_queue_entry_for(&player.id))
            .is_some_and(|entry| matches!(entry.status, crate::app::StrategyQueueStatus::Queued));
        if queued {
            lines.push(Line::from(Span::styled(
                "· QUEUED FOR DEEP STRATEGY",
                Style::default().add_modifier(Modifier::BOLD),
            )));
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled(
                "One full evaluation runs at a time.",
                Style::default().add_modifier(Modifier::DIM),
            )));
        } else {
            lines.push(Line::from(Span::styled(
                format!("{} FINDING BEST STRATEGY", app.strategy_view_spinner()),
                Style::default().add_modifier(Modifier::BOLD),
            )));
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled(
            ...
            lines.push(Line::from(format!(
                ...
                app.strategy_view_strategy_count(),
                app.strategy_view_stage_label(),
            )));
            lines.push(Line::from(""));
            lines.push(Line::from(Span::styled(
                format!("elapsed {:.1}s", app.strategy_view_elapsed_seconds()),
                Style::default().add_modifier(Modifier::DIM),
            )));
        }
    } else if let Some(plan) = app.strategy_view_plan() {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            "DURANT STRATEGY",
            Style::default().add_modifier(Modifier::DIM),
        ...
        let mut alternative_names: Vec<&str> = Vec::new();
        for alternative in &plan.j_alternatives {
            let name = alternative.build_name.as_str();
            if name == plan.build_name.as_str() || alternative_names.contains(&name) {
                continue;
            }
            alternative_names.push(name);
            if alternative_names.len() == 2 {
                break;
            }
        }
        ...
        if !alternative_names.is_empty() {
            ...
            for name in alternative_names {
                if let Some(alternative) = plan
                    .j_alternatives
                    .iter()
                    .find(|alternative| alternative.build_name == name)
                {
                    lines.push(Line::from(format!(
                        "{:>5.1}%  {}",
    ...
    } else if let Some(error) = app.strategy_view_error() {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            error.to_string(),
            Style::default().add_modifier(Modifier::DIM),
...
fn render_projected_roster(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(plan) = app.strategy_view_plan() {
        let mut slot = 1usize;
    ...
    } else if app.strategy_view_loading() {
        for player_id in app.own_roster_ids() {
            let name = app
                .durant
                .score_for(&player_id)
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            format!("{} completing the roster...", app.strategy_view_spinner()),
            Style::default().add_modifier(Modifier::DIM),
        )));
    } else {
        lines.push(Line::from(Span::styled(
...
fn render_category_workbench(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let plan = app.strategy_view_plan();

    let best_header = if app.strategy_view_loading() {
        format!("BEST CASE {}", app.strategy_view_spinner())
    } else {
        "BEST CASE".to_string()
    };
    ...
    for index in 0..9 {
        ...
        lines.push(Line::from(format!(
            ...
            if best_case.is_none() && app.strategy_view_loading() {
                format!("{} computing", app.strategy_view_spinner())
            } else {
    ...
    if let Some(score) = live_score {
        ...
        lines.push(Line::from(format!(
            ...
            final_h
                ...
                .unwrap_or_else(|| if app.strategy_view_loading() {
                    app.strategy_view_spinner().to_string()
                } else {
                    "—".to_string()
                }),
        )));
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(Span::styled(
        "Marks show category strength vs 50%.",
        Style::default().add_modifier(Modifier::DIM),
    )));
...
fn selected_price(app: &App, player_id: &crate::player::PlayerId) -> Option<u16> {
    app.strategy_assumed_price_for(player_id)
        .or_else(|| {
