...
impl App {
    ...
    pub fn refresh_market_board(&mut self) {
        ...
        if matches!(self.session_phase, SessionPhase::LiveDraft) {
            ...
            // Immediately queue one expensive candidate-free solve for the
            // CURRENT draft state. It shares the same single worker as manual
            // player evaluations and therefore never multiplies beam jobs.
    ...
    fn enqueue_team_strategy_plan(&mut self) {
        ...
        self.strategy_queue.insert(
            ...
            StrategyQueueEntry {
                ...
                player_name: "TEAM PLAN".to_string(),
                assumed_price: 0,
                status: StrategyQueueStatus::Queued,
                plan: None,
                elapsed_ms: None,
            ...
            match receiver.try_recv() {
                Ok(result) => {
                    ...
                    if result.generation == self.strategy_queue_generation {
                        ...
                        let team_plan = if matches!(result.kind, StrategyQueueKind::TeamPlan) {
                            result.plan.clone()
                        } else {
                            None
                        };
                        ...
                        // Once the expensive current-team solve is ready, let the
                        // Live PLAYER FIT table use its higher-quality strategy.
