...
impl DurantModel {
    ...
    /// This is the live/runtime counterpart to the offline j census: callers
    /// can swap the JSON bank without changing DURANT code.
    pub fn roster_plan_with_strategy_weights(
        ...
        let mut rollouts = self.top_auction_rollouts(
            None,
            own_sum,
            own_open_slots,
            own_budget_remaining,
        ...
        let coarse_best = rollouts.remove(0);
        let best = self.refine_rollout_with_dynamic_future_market(
            own_sum,
            own_roster.len(),
            own_open_slots,
            own_budget_remaining,
            &coarse_best.strategy,
            &[],
            candidates,
            opponent_rosters,
            opponent_budgets_remaining,
        ...
        let second_probability = rollouts
            .first()
            .map(|result| result.evaluation.matchup_win_probability)
            .unwrap_or(coarse_best.evaluation.matchup_win_probability);
        let alternatives = rollouts
            .iter()
            .map(|result| StrategyAlternative {
                j_name: describe_strategy(
