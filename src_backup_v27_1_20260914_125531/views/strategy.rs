...
fn render_roster_outlook(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let profile = app.durant.roster_x_profile(&roster);
    ...
    let mut lines = vec![
        ...
        Line::from(""),
        Line::from(format!(
            "Roster {roster_count}/13   Budget ${budget}   Max ${max_bid}"
        )),
    ];

    if let Some(plan) = &app.live_roster_plan {
        lines.push(Line::from(format!(
            "Current H {:>5.1}%   Projected H {:>5.1}%",
...
fn render_computer_plan(frame: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(plan) = &app.live_roster_plan {
        ...
        for name in plan.projected_future_player_names.iter().take(6) {
            lines.push(Line::from(format!("  {name}")));
        }
        ...
        lines.push(Line::from(Span::styled(
            "No runtime strategy bank loaded.",
            Style::default().add_modifier(Modifier::DIM),
        )));
    }
