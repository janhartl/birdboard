...
///
/// Crucially this does NOT use ESPN prices and it does NOT use the legacy
/// linear G->$ market. Each synthetic manager gets one strategy-bank profile.
/// Its player utility combines two SCALE-FREE rankings:
///
///   1. roster-dependent immediate H gain
///   2. category-G value under that manager's strategy weights
///
/// For each open roster slot the manager allocates its entire remaining
/// discretionary budget over its preferred `open_slots` players according to
/// utility surplus over the next-best alternative. Those allocations are max
/// bids. Every sale then clears at second-highest max bid + $1.
///
/// Repeating complete 13x13 auctions with heterogeneous managers produces an
/// endogenous sale-price distribution. The saved equilibrium weight is the
/// unconditional expected discretionary clearing value (sale frequency matters),
/// used as a PRIOR SHAPE only: live BirdBoard redistributes the actual room's remaining
/// dollars over that shape, so real auction inflation/deflation still matters.
#[test]
#[ignore = "expensive one-time 13x13 equilibrium market simulation"]
fn build_independent_equilibrium_market() -> Result<()> {
    ...
    println!("\nBirdBoard independent equilibrium market");
    println!(
        "{} rooms x {} teams x {} roster slots; ${} per team",
        rooms, TEAM_COUNT, ROSTER_SIZE, STARTING_BUDGET
    );
    println!("player pool: {}", candidate_pool.len());
    println!("strategy profiles available: {}", strategy_weights.len());
    println!(
        "pricing signal: roster-dependent H rank + strategy-weighted category-G rank; NO ESPN inputs and NO legacy G/$ prices"
    );
    println!("auction clearing: rotating nominations, second-highest max bid + $1\n");

    let model = &app.durant;
    let outcomes = (0..rooms)
        .into_par_iter()
        .map(|room| {
            let seed = 0xE9A1_2027u64.wrapping_add((room as u64).wrapping_mul(104_729));
            simulate_independent_equilibrium_room(model, &candidate_pool, strategy_weights, seed)
        })
        .collect::<Vec<_>>();
    ...
    for player_id in &candidate_pool {
        ...
        let sale_rate = observations as f64 / rooms as f64;
    ...
    let team_samples = rooms * TEAM_COUNT;
    ...
    println!("\nTOP 30 INDEPENDENT CLEARING PRICES");
    println!(
        "{:<3} {:<25} {:>7} {:>7} {:>7} {:>11} {:>9}",
        "#", "player", "eq", "median", "mean", "IQR", "sale rate"
    );
    ...
    println!(
        "\nBirdBoard will now automatically use this curve as its Preparation/LIVE market prior."
    );
    println!(
        "The saved independent demand weights are re-scaled to the ACTUAL remaining league dollars after every real pick."
    );
    Ok(())
}
