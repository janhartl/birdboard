...
fn build_independent_equilibrium_market() -> Result<()> {
    ...
    println!("\nBirdBoard independent synthetic auction market v75");
    println!(
    ...
    println!(
        "bids: current-state H-indifference max bids; future opportunity cost uses the legacy replacement/G-$ market"
    );
    println!(
...
enum CompetitiveBirdBoardBidMode {
    ...
    /// ceiling comes from BirdBoard's independently simulated synthetic market.
...
fn independent_equilibrium_market_vs_espn() -> Result<()> {
    ...
    println!(
        "BirdBoard TEAM selection uses the independently generated synthetic market. Two matched variants differ ONLY in BirdBoard's bid ceiling: ESPN clearing estimate vs BirdBoard synthetic-market estimate."
    );
    println!(
    ...
    println!(
        "TEAM @ BirdBoard market is the end-to-end test of BirdBoard's independent synthetic market + TEAM strategy against the ESPN-valued field."
    );
    println!(
