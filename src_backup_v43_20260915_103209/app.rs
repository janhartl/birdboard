...
impl App {
        ...
        match key.code {
            ...
            KeyCode::Char('s')
                if matches!(&self.draft_mode, DraftMode::BrowsingPlayers)
                    && matches!(&self.session_phase, SessionPhase::LiveDraft) =>
            {
                // Strategy is intentionally accurate-first now. Open the screen
                // immediately, then build the full opponent-aware plan on a
                // worker thread so rendering/input never waits for ~15 seconds.
    ...
    pub fn start_full_strategy_plan_async(&mut self) {
        ...
        if self.strategy_full_plan_ready && !self.live_roster_plan_dirty {
            return;
        }
    ...
    pub fn poll_background_work(&mut self) {
        let received = match self.strategy_plan_rx.as_ref() {
