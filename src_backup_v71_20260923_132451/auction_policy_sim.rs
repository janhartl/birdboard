...
enum CompetitiveBirdBoardBidMode {
    ...
    /// ceiling comes from BirdBoard's own dynamic market model.
    NativeBirdBoardMarket,
}

impl CompetitiveBirdBoardBidMode {
    const ALL: [Self; 2] = [Self::EspnClearingMarket, Self::NativeBirdBoardMarket];

    fn label(self) -> &'static str {
        match self {
            Self::EspnClearingMarket => "TEAM @ ESPN market",
            Self::NativeBirdBoardMarket => "TEAM @ native market",
        }
    }
}
...
#[test]
#[ignore = "network + competitive salary-cap auction + realized weekly backtest"]
fn espn_competitive_auction_vs_birdboard_strategy() -> Result<()> {
    ...
    println!("\nCompetitive ESPN-market auction vs BirdBoard TEAM strategy");
    println!("Fetching current ESPN average salary-cap prices...");
    let espn_entries = fetch_current_espn_auction_values()?;
    let espn_prices = match_espn_prices_to_birdboard(&app, &espn_entries);
    println!(
    ...
    if candidate_pool.len() < TEAM_COUNT * ROSTER_SIZE {
        bail!(
            "only {} players overlap BirdBoard + ESPN + weekly history; need at least {}",
            candidate_pool.len(),
            TEAM_COUNT * ROSTER_SIZE
        );
    }
    ...
    let validation_weeks = choose_five_pool_calendar_weeks(&app, &candidate_pool, 0xA11C_7100u64)?;
    let rows_by_week = weekly_rows_by_week_for_sim(&app.stats.weekly);

    println!(
        "Using {} players with ESPN prices + weekly history.",
        candidate_pool.len()
    );
    println!("Matched realized-stat weeks: {:?}", validation_weeks);
    println!(
        "Auction model: rotating nominations, all teams submit a legal max bid, winner pays second-highest max + $1."
    );
    println!(
        "ESPN-bot ceilings use current ESPN average prices + live room inflation + mild stable roster preference."
    );
    println!(
        "Two BirdBoard variants are run on matched seeds: TEAM strategy at the ESPN clearing market, and TEAM strategy with BirdBoard's native market ceiling plus auction-budget pressure.\n"
    );
    ...
    println!("\nINTERPRETATION");
    println!(
        "TEAM @ ESPN market isolates PLAYER/ROSTER SELECTION: BirdBoard gets the same live clearing-price environment as the ESPN-value field."
    );
    println!(
        "TEAM @ native market is the tougher END-TO-END check: BirdBoard uses its own valuation, but dynamically raises willingness to pay when it is falling behind the auction's spending pace."
    );
    println!(
        "If ESPN-market wins but native-market does not, the strategy engine is working and market-price calibration is the bottleneck."
    );
    println!(
        "Weekly scoring uses realized source-season player weeks; missing rows count as zero production for that calendar matchup."
    );
...
fn birdboard_competitive_max_bid(
    ...
    let ceiling = match mode {
        ...
        CompetitiveBirdBoardBidMode::NativeBirdBoardMarket => {
            let native = native_prices.get(player_id).copied().unwrap_or(MIN_BID) as f64;

            // A static native valuation is not enough in a finite auction.
            // If BirdBoard keeps losing targets while the room spends down,
            // unused dollars become progressively less valuable. Raise the
            // ceiling only when BirdBoard is behind the room's spending pace.
            // This preserves the native ranking/value signal while preventing
            // the pathological "finish with $140" behaviour from the first
            // competitive validation.
            let spending_pressure = birdboard_native_spending_pressure(state);
            native * priority_multiplier * spending_pressure
        }
    };
...
fn birdboard_native_spending_pressure(state: &SimState) -> f64 {
    let total_slots = TEAM_COUNT * ROSTER_SIZE;
    let filled_slots = state.rosters.iter().map(Vec::len).sum::<usize>();
    let progress = filled_slots as f64 / total_slots.max(1) as f64;

    let bb_open = state.open_slots(0);
    if bb_open == 0 {
        return 1.0;
    }

    let bb_reserve = MIN_BID.saturating_mul(bb_open as u16);
    let bb_discretionary = state.budgets[0].saturating_sub(bb_reserve) as f64;
    let starting_discretionary =
        STARTING_BUDGET.saturating_sub(MIN_BID.saturating_mul(ROSTER_SIZE as u16)) as f64;

    // At 50% of all roster slots sold, a neutral spending pace has roughly
    // 50% of the original discretionary budget left. If BirdBoard has more,
    // it is behind pace and should tolerate a higher price on its chosen targets.
    let expected_discretionary = (starting_discretionary * (1.0 - progress)).max(MIN_BID as f64);
    let pace_pressure = (bb_discretionary / expected_discretionary).max(1.0);

    // Also react when BirdBoard has much more money per remaining slot than
    // the active room. This catches uneven stars-and-scrubs spending before
    // the global progress measure becomes extreme.
    let active_open_slots = (0..TEAM_COUNT)
        .map(|team| state.open_slots(team))
        .sum::<usize>();
    let active_budget = (0..TEAM_COUNT)
        .filter(|team| state.open_slots(*team) > 0)
        .map(|team| state.budgets[team] as f64)
        .sum::<f64>();
    let room_dollars_per_slot = if active_open_slots == 0 {
        MIN_BID as f64
    } else {
        active_budget / active_open_slots as f64
    };
    let bb_dollars_per_slot = state.budgets[0] as f64 / bb_open as f64;
    let relative_pressure = if room_dollars_per_slot <= f64::EPSILON {
        1.0
    } else {
        (bb_dollars_per_slot / room_dollars_per_slot).max(1.0)
    };

    // The cap prevents one missed stretch from turning into unconditional
    // overbidding, while 3.5x is enough to make a chronically under-spending
    // team re-enter realistic mid/late-auction bidding.
    pace_pressure.max(relative_pressure).clamp(1.0, 3.5)
}
