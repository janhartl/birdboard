...
use crate::equilibrium_market::{self, EquilibriumMarket};
...
const TEAM_PLAN_QUEUE_ID: &str = "__birdboard_team_plan__";

fn team_plan_queue_id() -> PlayerId {
    PlayerId(TEAM_PLAN_QUEUE_ID.to_string())
}
...
pub struct StrategyQueueEntry {
    ...
    /// TEAM PLAN only: while a player is being auctioned, the deep worker can
    /// solve the likely *next* market state by assuming this nominee disappears
    /// to an opponent at the competition price.  The existing fast plan stays
    /// in `plan` until the speculative deep solve finishes, so NEXT TARGETS is
    /// never blank.
    pub speculative_player_id: Option<PlayerId>,
    pub speculative_player_name: Option<String>,
    pub speculative_price: Option<u16>,
    /// Becomes true when the speculative nominee actually sells to an opponent.
    /// At that point the in-flight work is close enough to the real state to
    /// finish instead of throwing away 20-30 seconds of auction-time compute.
    pub speculation_confirmed: bool,
}

struct StrategyQueueJobResult {
    generation: u64,
    kind: StrategyQueueKind,
    player_id: PlayerId,
...
pub struct App {
    ...
    pub equilibrium_market: Option<EquilibriumMarket>,
...
impl App {
    pub fn new(stats: StatsBundle) -> Result<App> {
        ...
        let mut durant = DurantModel::from_stats(&stats, teams.len(), 13)?;
        ...
        // Production MARKET policy:
        //   1. Prefer the independently simulated synthetic auction curve.
        //   2. Reprice that relative demand curve against the live room after
        //      every pick.
        //   3. Fall back to the transparent legacy G-above-replacement curve
        //      only when the synthetic artifact is missing/invalid, or for an
        //      individual player absent from an otherwise usable artifact.
        //
        // Requiring at least one full league's worth of current-player overlap
        // prevents a stale/sparse CSV from silently becoming the production
        // market. The normal synthetic census saves the entire scoring pool,
        // so a healthy artifact should comfortably exceed this threshold.
        let equilibrium_market = match equilibrium_market::load_for_season(&stats.draft_season) {
            Ok(Some(market)) => {
                let current_ids = durant
                    .scores
                    .iter()
                ...
                let overlap = market
                    .prices
                    .keys()
                    .filter(|player_id| current_ids.contains(*player_id))
                    .count();
                let coverage_floor = (current_ids.len().saturating_mul(4) + 4) / 5;
                let minimum_overlap = durant
                    .reference_size
                    .max(coverage_floor)
                    .min(current_ids.len());
                ...
                if overlap < minimum_overlap {
                    eprintln!(
                        "BirdBoard: synthetic MARKET artifact is too sparse for production \
                         ({} current-player prices; need at least {}). Falling back to legacy G/$.",
                        overlap, minimum_overlap
                    );
                    None
                } else {
                    eprintln!(
                        "BirdBoard: MARKET = independent synthetic auction curve \
                         ({} current-player prices, {}/{} coverage); legacy G/$ remains the per-player fallback.",
                        overlap,
                        overlap,
                        current_ids.len()
                    );
            ...
            Ok(None) => {
                eprintln!(
                    "BirdBoard: no synthetic MARKET artifact for {}; using legacy G/$ fallback.",
                    stats.draft_season
                );
                None
            }
            Err(error) => {
                eprintln!(
                    "BirdBoard: ignoring invalid synthetic MARKET artifact ({error}); using legacy G/$ fallback."
                );
                None
            }
        };
        durant.set_equilibrium_prices(
            equilibrium_market
                .as_ref()
                .map(|market| market.prices.clone()),
        );
    ...
    pub fn record_draft(
        ...
        let preserve_speculative_team_plan =
            self.can_preserve_speculative_team_plan_for_sale(&player_id, team_id);
        ...
        self.player_form = None;
        self.team_input = None;
        if preserve_speculative_team_plan {
            self.confirm_speculative_team_plan_sale(&player_id);
            self.refresh_market_board_internal(true);
        } else {
            self.refresh_market_board();
        }
    ...
    pub fn begin_drafting_selected_player(&mut self) {
        ...
        if self.selected_player.is_some() && !self.teams.is_empty() {
            ...
            self.draft_price_input.clear();
            self.start_speculative_team_plan_for_nomination();
        }
    }
    ...
    pub fn escape_drafting_selected_player(&mut self) {
        ...
        self.invalidate_full_pricing_frontier();
        self.cancel_speculative_team_plan();
    }
    ...
    /// Re-install the production MARKET prior after DURANT is rebuilt.
    ///
    /// Projection/team edits reconstruct `DurantModel`; without this step the
    /// new model silently loses its synthetic market and falls back to legacy
    /// G/$ for the rest of the session. Keeping this in one helper makes the
    /// production price source sticky across every normal rebuild.
    fn restore_production_market_prior(&mut self) {
        self.durant.set_equilibrium_prices(
            self.equilibrium_market
                .as_ref()
                .map(|market| market.prices.clone()),
        );
    }
    ...
    pub fn refresh_market_board(&mut self) {
        self.refresh_market_board_internal(false);
    }

    fn refresh_market_board_internal(&mut self, preserve_confirmed_team_plan: bool) {
        self.invalidate_strategy_full_plan();
        self.invalidate_full_pricing_frontier();
        if !preserve_confirmed_team_plan {
        ...
        if matches!(self.session_phase, SessionPhase::LiveDraft) {
            ...
            if preserve_confirmed_team_plan {
                // If the nominee really sold to an opponent, keep the in-flight
                // speculative deep solve. While it finishes, refresh its visible
                // fallback targets from the exact real state we just recorded.
                let fast_plan = self.live_roster_plan.clone();
                let fast_ms = self.live_roster_plan_ms;
                if let Some(entry) = self
                    .strategy_queue
                    .iter_mut()
                    .find(|entry| matches!(entry.kind, StrategyQueueKind::TeamPlan))
                {
                    if matches!(entry.status, StrategyQueueStatus::Running) {
                        entry.plan = fast_plan;
                        entry.elapsed_ms = fast_ms;
                    }
                } else {
                    self.enqueue_team_strategy_plan();
                }
            } else {
                // Always expose a usable recommendation immediately. Nomination
                // can later upgrade this fast plan in the background.
                self.enqueue_team_strategy_plan();
            }
        }
    ...
    pub fn strategy_queue_entry_for(&self, player_id: &PlayerId) -> Option<&StrategyQueueEntry> {
        self.strategy_queue.iter().find(|entry| {
            matches!(entry.kind, StrategyQueueKind::Player) && &entry.player_id == player_id
        })
    }

    pub fn team_strategy_queue_entry(&self) -> Option<&StrategyQueueEntry> {
        self.strategy_queue
            .iter()
            .find(|entry| matches!(entry.kind, StrategyQueueKind::TeamPlan))
    }

    /// The current recommendation list. Prefer the TEAM PLAN carried by the
    /// queue (fast baseline or speculative deep result) and fall back to the
    /// live current-roster plan. Prices are always taken from the *real* current
    /// market, even when the ordering came from a speculative next-state solve.
    pub fn next_targets(&self, limit: usize) -> Vec<(String, u16)> {
        let plan = self
            .team_strategy_queue_entry()
            .and_then(|entry| entry.plan.as_ref())
            .or(self.live_roster_plan.as_ref());
        let Some(plan) = plan else {
            return Vec::new();
        };

        plan.projected_future_players
            .iter()
            .filter(|player_id| self.draft_pick_for_player(player_id).is_none())
            .filter_map(|player_id| {
                ...
                Some((player.display_name().to_string(), price))
            })
            .take(limit)
            .collect()
    }
    ...
    pub fn toggle_selected_strategy_queue(&mut self) {
        ...
        if let Some(index) = self.strategy_queue.iter().position(|entry| {
            matches!(entry.kind, StrategyQueueKind::Player) && entry.player_id == player_id
        }) {
            let running = matches!(
    ...
    fn enqueue_strategy_player(&mut self, player_id: PlayerId) {
        ...
        self.strategy_queue.push(StrategyQueueEntry {
            ...
            speculative_player_id: None,
            speculative_player_name: None,
            speculative_price: None,
            speculation_confirmed: false,
        });
        self.start_next_strategy_queue_job();
    }

    fn enqueue_team_strategy_plan(&mut self) {
        if !matches!(self.session_phase, SessionPhase::LiveDraft)
            || self.team_strategy_queue_entry().is_some()
        {
            return;
        }

        // NEXT TARGETS is always populated immediately from the fast current-
        // roster plan.  A nomination can later upgrade this same entry with a
        // speculative full-rational solve for the likely next market state.
        let plan = self.live_roster_plan.clone();
        let has_plan = plan.is_some();
        self.strategy_queue.insert(
            0,
            StrategyQueueEntry {
                kind: StrategyQueueKind::TeamPlan,
                player_id: team_plan_queue_id(),
                player_name: "NEXT TARGETS".to_string(),
                assumed_price: 0,
                status: if has_plan {
                    StrategyQueueStatus::Ready
                } else {
                ...
                plan,
                elapsed_ms: self.live_roster_plan_ms,
                error: (!has_plan).then(|| "Live Team Plan found no valid roster plan".to_string()),
                stage: self.active_strategy_stage,
                strategy_count: self.active_strategy_weights.len(),
                speculative_player_id: None,
                speculative_player_name: None,
    ...
    /// During a bidding war, spend the otherwise dead auction time solving the
    /// likely *next* state: assume the nominee is bought by an opponent at the
    /// competition price.  Keep the current fast plan visible until the deep
    /// result arrives, so NEXT TARGETS never disappears.
    fn start_speculative_team_plan_for_nomination(&mut self) {
        let Some(player) = self.selected_player_ref() else {
            return;
        };
        let player_id = player.id.clone();
        ...
        let player_name = player.display_name().to_string();
        let competition_price = self
            .nomination_advice
            .as_ref()
            .map(|advice| advice.market_price)
            .or_else(|| {
        ...
        // A prior speculative solve should normally have finished before the
        // next nomination. If it has not, prefer the new nomination: its 30s
        // bidding window is the compute opportunity that matters now.
        let team_plan_id = team_plan_queue_id();
        if self.strategy_queue_active_player.as_ref() == Some(&team_plan_id) {
            if let Some(cancel) = self.strategy_queue_cancel.take() {
                cancel.store(true, Ordering::Relaxed);
            }
            self.strategy_queue_generation = self.strategy_queue_generation.wrapping_add(1);
        ...
        let fallback_plan = self.live_roster_plan.clone().or_else(|| {
            self.team_strategy_queue_entry()
        ...
        if let Some(index) = self
            .strategy_queue
            .iter()
            .position(|entry| matches!(entry.kind, StrategyQueueKind::TeamPlan))
        {
            let entry = &mut self.strategy_queue[index];
            entry.player_name = "NEXT TARGETS".to_string();
            entry.assumed_price = competition_price;
            entry.status = StrategyQueueStatus::Queued;
            entry.plan = fallback_plan;
            entry.elapsed_ms = None;
            entry.error = None;
            entry.stage = self.active_strategy_stage;
            entry.strategy_count = self.active_strategy_weights.len();
            entry.speculative_player_id = Some(player_id);
            entry.speculative_player_name = Some(player_name);
            entry.speculative_price = Some(competition_price);
            entry.speculation_confirmed = false;
        } else {
            self.strategy_queue.insert(
                0,
                StrategyQueueEntry {
                    kind: StrategyQueueKind::TeamPlan,
                    player_id: team_plan_id,
                    player_name: "NEXT TARGETS".to_string(),
                    assumed_price: competition_price,
                    status: StrategyQueueStatus::Queued,
                    plan: fallback_plan,
                    elapsed_ms: None,
                    error: None,
                    stage: self.active_strategy_stage,
                    strategy_count: self.active_strategy_weights.len(),
                    speculative_player_id: Some(player_id),
                    speculative_player_name: Some(player_name),
                    speculative_price: Some(competition_price),
                    speculation_confirmed: false,
                },
            );
        }
    ...
    fn cancel_speculative_team_plan(&mut self) {
        let Some(index) = self.strategy_queue.iter().position(|entry| {
            matches!(entry.kind, StrategyQueueKind::TeamPlan)
                && entry.speculative_player_id.is_some()
        }) else {
            return;
        };
    ...
    fn can_preserve_speculative_team_plan_for_sale(
        &self,
        player_id: &PlayerId,
        team_id: TeamId,
    ) -> bool {
        ...
        self.team_strategy_queue_entry().is_some_and(|entry| {
            entry.speculative_player_id.as_ref() == Some(player_id)
                && matches!(
                    entry.status,
                    StrategyQueueStatus::Running | StrategyQueueStatus::Ready
                )
        })
    }

    fn confirm_speculative_team_plan_sale(&mut self, player_id: &PlayerId) {
        // Candidate-specific queue work is stale after any real sale. Preserve
        // only the speculative NEXT TARGETS job/result whose nominee was the
        // player that actually disappeared to an opponent.
        let team_plan_id = team_plan_queue_id();
        if self.strategy_queue_active_player.as_ref() != Some(&team_plan_id) {
            if let Some(cancel) = self.strategy_queue_cancel.take() {
                cancel.store(true, Ordering::Relaxed);
            }
            if self.strategy_queue_rx.is_some() || self.strategy_queue_active_player.is_some() {
                self.strategy_queue_generation = self.strategy_queue_generation.wrapping_add(1);
            }
            self.strategy_queue_rx = None;
            self.strategy_queue_active_player = None;
        ...
        self.strategy_queue
            .retain(|entry| matches!(entry.kind, StrategyQueueKind::TeamPlan));
        if let Some(entry) = self.strategy_queue.iter_mut().find(|entry| {
            matches!(entry.kind, StrategyQueueKind::TeamPlan)
                && entry.speculative_player_id.as_ref() == Some(player_id)
        }) {
            entry.speculation_confirmed = true;
        }
    }
    ...
    fn start_next_strategy_queue_job(&mut self) {
        ...
        let kind = self.strategy_queue[queue_index].kind;
        ...
        let speculative_player_id = self.strategy_queue[queue_index]
            .speculative_player_id
            .clone();
        let speculative_price = self.strategy_queue[queue_index].speculative_price;
        ...
        let current_budget = self
            ...
            .map(|team| team.budget as u16)
            .unwrap_or(AuctionConfig::default().starting_budget);
        let mut own_budget = current_budget;

        if matches!(kind, StrategyQueueKind::Player) {
            ...
            own_budget = current_budget - assumed_price;
        }
        ...
        let candidates = self
            ...
            .filter(|player| !matches!(kind, StrategyQueueKind::Player) || player.id != target_id)
            .filter(|player| speculative_player_id.as_ref() != Some(&player.id))
        ...
        let opponent_teams = self
            ...
            .filter(|team| team.id != self.user_team_id)
            .collect::<Vec<_>>();
        let mut opponent_rosters = opponent_teams
            .iter()
            .map(|team| self.roster_ids_for_team(team.id))
            .collect::<Vec<_>>();
        let mut opponent_budgets = opponent_teams
            .iter()
            .map(|team| team.budget as u16)
            .collect::<Vec<_>>();

        // Speculative NEXT TARGETS: remove the current nominee and assign him
        // to the most plausible opponent (the eligible team with the largest
        // remaining budget) at the current competition price.  The exact buyer
        // matters much less than the key fact that the nominee is no longer in
        // our future player pool.
        if matches!(kind, StrategyQueueKind::TeamPlan)
            && let Some(speculative_id) = speculative_player_id.as_ref()
        {
            let sale_price = speculative_price.unwrap_or(assumed_price);
            let mut buyer_index = None;
            let mut best_budget = 0_u16;
            for index in 0..opponent_rosters.len() {
                let budget = opponent_budgets[index];
                if opponent_rosters[index].len() < self.durant.team_size()
                    && budget >= sale_price
                    && (buyer_index.is_none() || budget > best_budget)
                {
                    buyer_index = Some(index);
                    best_budget = budget;
                }
            }
            if buyer_index.is_none() {
                for (index, budget) in opponent_budgets.iter().copied().enumerate() {
                    if buyer_index.is_none() || budget > best_budget {
                        buyer_index = Some(index);
                        best_budget = budget;
                    }
                }
            ...
            if let Some(index) = buyer_index {
                opponent_rosters[index].push(speculative_id.clone());
                opponent_budgets[index] = opponent_budgets[index].saturating_sub(sale_price);
            }
        }
        ...
        thread::spawn(move || {
            ...
            // half of the logical CPUs so the TUI and Live Board stay responsive.
            ...
            let error = plan.is_none().then(|| match kind {
                StrategyQueueKind::TeamPlan => {
                    "Full Team Plan found no valid roster plan".to_string()
                }
                StrategyQueueKind::Player => {
                    "Full Strategy evaluation found no valid roster plan".to_string()
                }
            });
    ...
    fn select_strategy_queue_entry(&mut self, direction: i32) {
        let player_entries = self
            .strategy_queue
            .iter()
            .filter(|entry| matches!(entry.kind, StrategyQueueKind::Player))
            .collect::<Vec<_>>();
        ...
        let current_index = current_id
            ...
            .unwrap_or(0);
        let len = player_entries.len() as i32;
        let next = (current_index as i32 + direction).rem_euclid(len) as usize;
        let target_id = player_entries[next].player_id.clone();
        self.selected_player = self
            .players
            .iter()
            ...
            match receiver.try_recv() {
                Ok(result) => {
                    ...
                    if result.generation == self.strategy_queue_generation {
                        ...
                        let team_plan_should_upgrade_live =
                            matches!(result.kind, StrategyQueueKind::TeamPlan)
                                && self
                                    .strategy_queue
                                    .iter()
                                    .find(|entry| {
                                    ...
                                    .is_some_and(|entry| {
                                        entry.speculative_player_id.is_none()
                                            || entry.speculation_confirmed
                                    });
                        let team_plan = if team_plan_should_upgrade_live {
                            result.plan.clone()
                        } else {
                            None
                        };
                        if let Some(entry) = self.strategy_queue.iter_mut().find(|entry| {
                            entry.kind == result.kind && entry.player_id == result.player_id
                        }) {
                        ...
                        // A speculative plan may drive NEXT TARGETS while the
                        // nominee is still bidding, but it must not rewrite the
                        // current-state PLAYER FIT strategy until that sale is
                        // actually confirmed to an opponent.
                        if let Some(plan) = team_plan {
                            self.live_roster_plan = Some(plan);
    ...
    fn save_projection_form(&mut self, show_status: bool) -> Result<()> {
        ...
        self.restore_production_market_prior();
