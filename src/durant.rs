...
impl DurantModel {
    ...
    /// Production MARKET board for the current auction state. This does not
    /// run any j-search or H-score rollout.
    ///
    /// Price-source priority is deliberate:
    /// 1. an ABSOLUTE external table installed by a validation harness;
    /// 2. BirdBoard's independent synthetic auction curve (normal production);
    /// 3. the legacy linear G-above-replacement curve as bootstrap/fallback.
    ///
    /// `estimate_market_economy_from_totals` owns that dispatch so every
    /// caller -- Live, Strategy, nomination advice, Max Bid, and simulations --
    /// shares one market implementation.
    pub fn market_board(
        &self,
        own_roster: &[PlayerId],
        own_budget_remaining: u16,
