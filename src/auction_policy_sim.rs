...
fn market_for_team(
    ...
    // Use the exact same MARKET path as production BirdBoard. App::new installs
    // the independent synthetic curve when a valid artifact exists; offline
    // market generation explicitly clears it, and validation harnesses may
    // install an absolute external prior on a cloned model. Keeping one entry
    // point prevents simulation/production pricing semantics from drifting.
