...
fn independent_equilibrium_market_vs_espn() -> Result<()> {
    ...
    println!(
        "ESPN opponents use current ESPN averages + relative live room inflation anchored to 1.0 + team-specific finite-roster budget pressure that is zero at the opening state and rises only when a bot risks stranding money."
    );
    println!(
    ...
    println!(
        "Auction semantics v81: MARKET predicts future cost; ESPN opening dollars remain absolute; opponent bots add only endogenous late-draft budget pressure from their own remaining cash/slots; Max Bid is H-indifference, target membership never caps bidding, off-plan bargains may be bought, and every nomination carries the nominator's mandatory $1 opening bid."
    );
    println!(
...
fn print_espn_room_calibration(
    ...
    println!(
        "all-ESPN full-room endgame ({} deterministic rooms): avg $left/team {:.1} | max $left {} | avg room spend ${:.0}",
        endgame_samples,
...
///
/// The pressure is deliberately team-local:
///   * 1.0 when the manager can still deploy its budget at ESPN-like prices;
///   * gradually blended in as roster slots disappear;
///   * fully reflects the cash/alternative ratio on the final roster slot.
///
...
fn espn_bot_budget_pressure(
    ...
    let urgency = if ROSTER_SIZE <= 1 {
        1.0
    } else {
        let progress = filled as f64 / (ROSTER_SIZE - 1) as f64;
        // Little intervention early; rapidly expose the true shadow value of
        // stranded dollars in the final third of the roster.
